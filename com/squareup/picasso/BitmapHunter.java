...
class BitmapHunter implements Runnable {
    ...
    private static final RequestHandler ERRORING_HANDLER = new C39702();
    private static final ThreadLocal<StringBuilder> NAME_BUILDER = new C39691();
    ...
    static class C39691 extends ThreadLocal<StringBuilder> {
        C39691() {
        }
    ...
    static class C39702 extends RequestHandler {
        C39702() {
        }
    ...
    private Picasso$Priority computeNewPriority() {
        ...
        if (this.action != null) {
            picasso$Priority = this.action.getPriority();
        }
        if (hasMultiple) {
            int n = this.actions.size();
            for (int i = 0; i < n; i++) {
                Picasso$Priority actionPriority = ((Action) this.actions.get(i)).getPriority();
                if (actionPriority.ordinal() > picasso$Priority.ordinal()) {
