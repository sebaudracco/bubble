    ...
    public static byte[] compress(String urlString) throws MalformedURLException {
        if (urlString != null) {
            ...
            if (urlMatcher.matches()) {
                ...
                if (urlMatcher.group(1).toLowerCase(Locale.getDefault()).equalsIgnoreCase("http")) {
                    byteBuffer[0] = haswww ? (byte) 0 : (byte) 2;
                } else {
                    byteBuffer[0] = haswww ? (byte) 1 : (byte) 3;
                }
                int byteBufferIndex = 0 + 1;
                String[] domains = new String(urlMatcher.group(3).getBytes()).toLowerCase(Locale.getDefault()).split(Pattern.quote("."));
