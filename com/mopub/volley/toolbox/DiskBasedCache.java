...
public class DiskBasedCache implements Cache {
    ...
    static class CacheHeader {
        ...
        public boolean writeHeader(OutputStream os) {
            try {
                ...
                VolleyLog.m12071d("%s", e.toString());
                return false;
            }
        }
    }
    ...
    public synchronized void clear() {
        ...
        this.mEntries.clear();
        this.mTotalSize = 0;
        VolleyLog.m12071d("Cache cleared.", new Object[0]);
    }
            ...
            if (entry2 != null) {
                ...
                try {
                    ...
                    try {
                        ...
                        cis = cis2;
                        try {
                            VolleyLog.m12071d("%s: %s", file.getAbsolutePath(), e.toString());
                            remove(key);
                            if (cis != null) {
                                try {
                                    cis.close();
    ...
    public synchronized void initialize() {
        ...
        if (this.mRootDirectory.exists()) {
            ...
            VolleyLog.m12072e("Unable to create cache dir %s", this.mRootDirectory.getAbsolutePath());
        }
        return;
        if (fis != null) {
            try {
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (e.writeHeader(fos)) {
                ...
                fos.close();
                VolleyLog.m12071d("Failed to write header for %s", file.getAbsolutePath());
                throw new IOException();
            }
        } catch (IOException e2) {
            if (!file.delete()) {
                VolleyLog.m12071d("Could not clean up file %s", file.getAbsolutePath());
            }
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            VolleyLog.m12071d("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        if (this.mTotalSize + ((long) neededSpace) >= ((long) this.mMaxCacheSizeInBytes)) {
            if (VolleyLog.DEBUG) {
                VolleyLog.m12074v("Pruning old cache entries.", new Object[0]);
            }
            ...
            while (iterator.hasNext()) {
                ...
                if (getFileForKey(e.key).delete()) {
                    ...
                    VolleyLog.m12071d("Could not delete cache entry for key=%s, filename=%s", e.key, getFilenameForKey(e.key));
                }
                iterator.remove();
                prunedFiles++;
            ...
            if (VolleyLog.DEBUG) {
                VolleyLog.m12074v("pruned %d files, %d bytes, %d ms", Integer.valueOf(prunedFiles), Long.valueOf(this.mTotalSize - before), Long.valueOf(SystemClock.elapsedRealtime() - startTime));
            }
        }
    }
