...
public class CencMp4TrackImplImpl extends Mp4TrackImpl implements CencEncryptedTrack {
    ...
    private class FindSaioSaizPair {
        ...
        public FindSaioSaizPair invoke() {
            ...
            if ($assertionsDisabled || saizs.size() == saios.size()) {
                ...
                int i = 0;
                while (i < saizs.size()) {
                    if ((this.saiz == null && ((SampleAuxiliaryInformationSizesBox) saizs.get(i)).getAuxInfoType() == null) || C1681C.CENC_TYPE_cenc.equals(((SampleAuxiliaryInformationSizesBox) saizs.get(i)).getAuxInfoType())) {
                        this.saiz = (SampleAuxiliaryInformationSizesBox) saizs.get(i);
                    } else if (this.saiz != null && this.saiz.getAuxInfoType() == null && C1681C.CENC_TYPE_cenc.equals(((SampleAuxiliaryInformationSizesBox) saizs.get(i)).getAuxInfoType())) {
                        this.saiz = (SampleAuxiliaryInformationSizesBox) saizs.get(i);
                    } else {
                        throw new RuntimeException("Are there two cenc labeled saiz?");
                    }
                    if ((this.saio == null && ((SampleAuxiliaryInformationOffsetsBox) saios.get(i)).getAuxInfoType() == null) || C1681C.CENC_TYPE_cenc.equals(((SampleAuxiliaryInformationOffsetsBox) saios.get(i)).getAuxInfoType())) {
                        this.saio = (SampleAuxiliaryInformationOffsetsBox) saios.get(i);
                    } else if (this.saio != null && this.saio.getAuxInfoType() == null && C1681C.CENC_TYPE_cenc.equals(((SampleAuxiliaryInformationOffsetsBox) saios.get(i)).getAuxInfoType())) {
                        this.saio = (SampleAuxiliaryInformationOffsetsBox) saios.get(i);
                    } else {
    ...
    public CencMp4TrackImplImpl(String name, TrackBox trackBox, IsoFile... fragments) throws IOException {
        ...
        if ($assertionsDisabled || (schm != null && (schm.getSchemeType().equals(C1681C.CENC_TYPE_cenc) || schm.getSchemeType().equals(C1681C.CENC_TYPE_cbc1)))) {
