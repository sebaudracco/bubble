...
public class OneJpegPerIframe extends AbstractTrack {
    ...
    class C28871 extends AbstractList<Sample> {
        C28871() {
        }
    ...
    public OneJpegPerIframe(String name, File[] jpegs, Track alignTo) throws IOException {
        ...
        if (earliestTrackPresentationTime < 0.0d) {
            getEdits().add(new Edit((long) ((-earliestTrackPresentationTime) * ((double) getTrackMetaData().getTimescale())), getTrackMetaData().getTimescale(), MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE, ((double) getDuration()) / ((double) getTrackMetaData().getTimescale())));
        } else if (earliestTrackPresentationTime > 0.0d) {
            getEdits().add(new Edit(-1, getTrackMetaData().getTimescale(), MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE, earliestTrackPresentationTime));
            getEdits().add(new Edit(0, getTrackMetaData().getTimescale(), MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE, ((double) getDuration()) / ((double) getTrackMetaData().getTimescale())));
        }
    }
