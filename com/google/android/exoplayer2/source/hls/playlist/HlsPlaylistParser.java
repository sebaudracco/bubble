...
public final class HlsPlaylistParser implements Parser<HlsPlaylist> {
    ...
    private static HlsMasterPlaylist parseMasterPlaylist(LineIterator iterator, String baseUri) throws IOException {
        ...
        while (iterator.hasNext()) {
            ...
            if (line.startsWith(TAG_MEDIA)) {
                ...
                switch (obj) {
                    case null:
                        Format format = Format.createAudioContainerFormat(id, "application/x-mpegURL", null, null, -1, -1, -1, null, selectionFlags, language);
                        if (uri != null) {
                        ...
                        break;
                    case 1:
                        subtitles.add(new HlsUrl(uri, Format.createTextContainerFormat(id, "application/x-mpegURL", "text/vtt", null, -1, selectionFlags, language)));
                        break;
                    case 2:
                        String mimeType;
            ...
            if (line.startsWith(TAG_STREAM_INF)) {
                ...
                if (variantUrls.add(line)) {
                    variants.add(new HlsUrl(line, Format.createVideoContainerFormat(Integer.toString(variants.size()), "application/x-mpegURL", null, codecs, bitrate, width, height, -1.0f, null, 0)));
                }
            }
        }
        if (noClosedCaptions) {
    ...
    private static HlsMediaPlaylist parseMediaPlaylist(LineIterator iterator, String baseUri) throws IOException {
        ...
        return new HlsMediaPlaylist(playlistType, baseUri, startOffsetUs, playlistStartTimeUs, hasDiscontinuitySequence, playlistDiscontinuitySequence, mediaSequence, version, targetDurationUs, hasEndTag, playlistStartTimeUs != 0, initializationSegment, segments, dateRanges);
    }
