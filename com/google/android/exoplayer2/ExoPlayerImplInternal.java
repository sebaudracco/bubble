...
final class ExoPlayerImplInternal implements Callback, MediaPeriod.Callback, InvalidationListener, Listener {
    ...
    private void doSomeWork() throws ExoPlaybackException, IOException {
        ...
        if (allRenderersEnded && ((playingPeriodDurationUs == -9223372036854775807L || playingPeriodDurationUs <= this.playbackInfo.positionUs) && this.playingPeriodHolder.isLast)) {
            setState(4);
            stopRenderers();
        } else if (this.state == 2) {
    ...
    private void seekToInternal(SeekPosition seekPosition) throws ExoPlaybackException {
        ...
        boolean seekPositionAdjusted = seekPosition.windowPositionUs == -9223372036854775807L;
    ...
    private boolean isTimelineReady(long playingPeriodDurationUs) {
        return playingPeriodDurationUs == -9223372036854775807L || this.playbackInfo.positionUs < playingPeriodDurationUs || (this.playingPeriodHolder.next != null && this.playingPeriodHolder.next.prepared);
    }
    ...
    private void handleSourceInfoRefreshed(Pair<Timeline, Object> timelineAndManifest) throws ExoPlaybackException {
        ...
        if (oldTimeline == null) {
            if (this.pendingInitialSeekCount > 0) {
                ...
                if (this.timeline.isEmpty()) {
                    ...
                    Pair<Integer, Long> defaultPosition = getPeriodPosition(0, -9223372036854775807L);
        ...
        if (periodIndex == -1) {
            ...
            defaultPosition = getPeriodPosition(this.timeline.getPeriod(newPeriodIndex, this.period).windowIndex, -9223372036854775807L);
    ...
    private Pair<Integer, Long> resolveSeekPosition(SeekPosition seekPosition) {
        ...
        try {
            ...
            if (periodIndex != -1) {
                return getPeriodPosition(this.timeline.getPeriod(periodIndex, this.period).windowIndex, -9223372036854775807L);
            }
            return null;
    ...
    private void updatePeriods() throws ExoPlaybackException, IOException {
        ...
        if (this.playingPeriodHolder != null) {
            ...
            if (this.readingPeriodHolder.next != null && this.readingPeriodHolder.next.prepared) {
                ...
                boolean initialDiscontinuity = this.readingPeriodHolder.mediaPeriod.readDiscontinuity() != -9223372036854775807L;
                for (i = 0; i < this.renderers.length; i++) {
                    renderer = this.renderers[i];
    ...
    private void maybeUpdateLoadingPeriod() throws IOException {
        ...
        if (this.loadingPeriodHolder == null) {
            ...
            if (!this.loadingPeriodHolder.isLast && this.loadingPeriodHolder.isFullyBuffered()) {
                if (this.timeline.getPeriod(loadingPeriodIndex, this.period).getDurationUs() == -9223372036854775807L) {
                    return;
                }
        ...
        if (this.loadingPeriodHolder == null) {
            ...
            if (newLoadingPeriodIndex != this.timeline.getWindow(newLoadingWindowIndex, this.window).firstPeriodIndex) {
                ...
                Pair<Integer, Long> defaultPosition = getPeriodPosition(this.timeline, newLoadingWindowIndex, -9223372036854775807L, Math.max(0, (this.loadingPeriodHolder.getRendererOffset() + this.timeline.getPeriod(this.loadingPeriodHolder.index, this.period).getDurationUs()) - this.rendererPositionUs));
