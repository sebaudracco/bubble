...
final class zzeq implements Iterator<Entry<K, V>> {
    ...
    private zzeq(zzei com_google_android_gms_internal_clearcut_zzei) {
    ...
    public final void remove() {
        if (this.zzow) {
            ...
            if (this.pos < this.zzos.zzom.size()) {
                zzei com_google_android_gms_internal_clearcut_zzei = this.zzos;
                int i = this.pos;
                this.pos = i - 1;
                com_google_android_gms_internal_clearcut_zzei.zzal(i);
                return;
            }
            zzdw().remove();
            return;
