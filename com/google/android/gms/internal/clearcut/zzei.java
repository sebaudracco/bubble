...
class zzei<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public boolean equals(Object obj) {
        ...
        zzei com_google_android_gms_internal_clearcut_zzei = (zzei) obj;
        int size = size();
        if (size != com_google_android_gms_internal_clearcut_zzei.size()) {
            return false;
        }
        int zzdr = zzdr();
        if (zzdr != com_google_android_gms_internal_clearcut_zzei.zzdr()) {
            return entrySet().equals(com_google_android_gms_internal_clearcut_zzei.entrySet());
        }
        for (int i = 0; i < zzdr; i++) {
            if (!zzak(i).equals(com_google_android_gms_internal_clearcut_zzei.zzak(i))) {
                return false;
            }
        }
        return zzdr != size ? this.zzon.equals(com_google_android_gms_internal_clearcut_zzei.zzon) : true;
    }
    ...
    public final V zza(K k, V v) {
        ...
        if (this.zzom.size() == this.zzol) {
            zzep com_google_android_gms_internal_clearcut_zzep = (zzep) this.zzom.remove(this.zzol - 1);
            zzdv().put((Comparable) com_google_android_gms_internal_clearcut_zzep.getKey(), com_google_android_gms_internal_clearcut_zzep.getValue());
        }
