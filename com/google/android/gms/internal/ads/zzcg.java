package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.support.annotation.VisibleForTesting;
import android.support.v4.media.session.PlaybackStateCompat;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

public class zzcg extends zzcf {
    private static final String TAG = zzcg.class.getSimpleName();
    private static long startTime = 0;
    private static final Object zzqr = new Object();
    @VisibleForTesting
    private static boolean zzqs = false;
    protected boolean zzqt = false;
    private String zzqu;
    private boolean zzqv = false;
    private boolean zzqw = false;
    @VisibleForTesting
    private zzdi zzqx;

    protected zzcg(Context context, String str, boolean z) {
        super(context);
        this.zzqu = str;
        this.zzqt = z;
    }

    private static zzdf zza(zzcz com_google_android_gms_internal_ads_zzcz, MotionEvent motionEvent, DisplayMetrics displayMetrics) throws zzcw {
        Throwable e;
        Method zza = com_google_android_gms_internal_ads_zzcz.zza("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=");
        if (zza == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new zzdf((String) zza.invoke(null, new Object[]{motionEvent, displayMetrics}));
        } catch (IllegalAccessException e2) {
            e = e2;
            throw new zzcw(e);
        } catch (InvocationTargetException e3) {
            e = e3;
            throw new zzcw(e);
        }
    }

    protected static synchronized void zza(Context context, boolean z) {
        synchronized (zzcg.class) {
            if (!zzqs) {
                startTime = System.currentTimeMillis() / 1000;
                zzps = zzb(context, z);
                zzqs = true;
            }
        }
    }

    private static void zza(List<Callable<Void>> list) {
        if (zzps != null) {
            ExecutorService zzab = zzps.zzab();
            if (zzab != null && !list.isEmpty()) {
                try {
                    zzab.invokeAll(list, ((Long) zzkb.zzik().zzd(zznk.zzbaq)).longValue(), TimeUnit.MILLISECONDS);
                } catch (Throwable e) {
                    Log.d(TAG, String.format("class methods got exception: %s", new Object[]{zzdg.zza(e)}));
                }
            }
        }
    }

    private static zzcz zzb(Context context, boolean z) {
        if (zzps == null) {
            synchronized (zzqr) {
                if (zzps == null) {
                    zzcz zza = zzcz.zza(context, "fFhi0cTZpyVQYwMwl7BCfa0fa6esmkRUaNr4ktfJYZ8=", "", z);
                    if (zza.isInitialized()) {
                        if (((Boolean) zzkb.zzik().zzd(zznk.zzbaw)).booleanValue()) {
                            zza.zza("4o7tecxtkw7XaNt5hPj+0H1LvOi0SgxCIJTY9VcbazM/HSl/sFlxBFwnc8glnvoB", "RgSY6YxU2k1vLXOV3vapBnQwJDzYDlmX50wbm2tDcnw=", new Class[0]);
                        }
                        zza.zza("1QeH3Cf7T53ayw17Ebbo9YTdhU+IFx0X5nCtC5gZQym4uicOVPXxYWmMK9k58i8n", "bHJRpFJ+2R5LAbYQUBDMyfYpLd1oiGixlpIqMJOBQPY=", Context.class);
                        zza.zza("1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", Context.class);
                        zza.zza("+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", Context.class);
                        zza.zza("6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", Context.class);
                        zza.zza("f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", Context.class);
                        zza.zza("bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", Context.class);
                        zza.zza("ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", Context.class);
                        zza.zza("PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", Context.class);
                        zza.zza("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=", MotionEvent.class, DisplayMetrics.class);
                        zza.zza("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=", MotionEvent.class, DisplayMetrics.class);
                        zza.zza("1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", new Class[0]);
                        zza.zza("R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", new Class[0]);
                        zza.zza("uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", new Class[0]);
                        zza.zza("C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", new Class[0]);
                        zza.zza("YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", new Class[0]);
                        zza.zza("Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", new Class[0]);
                        zza.zza("LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        zza.zza("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", StackTraceElement[].class);
                        zza.zza("j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", View.class, DisplayMetrics.class);
                        zza.zza("WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", Context.class, Boolean.TYPE);
                        zza.zza("wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", View.class, Activity.class, Boolean.TYPE);
                        zza.zza("aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", Long.TYPE);
                    }
                    zzps = zza;
                }
            }
        }
        return zzps;
    }

    protected final long zza(StackTraceElement[] stackTraceElementArr) throws zzcw {
        Throwable e;
        Method zza = zzps.zza("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=");
        if (zza == null || stackTraceElementArr == null) {
            throw new zzcw();
        }
        try {
            return new zzcx((String) zza.invoke(null, new Object[]{stackTraceElementArr})).zzro.longValue();
        } catch (IllegalAccessException e2) {
            e = e2;
            throw new zzcw(e);
        } catch (InvocationTargetException e3) {
            e = e3;
            throw new zzcw(e);
        }
    }

    protected final zzba zza(Context context, View view, Activity activity) {
        List asList;
        zzba com_google_android_gms_internal_ads_zzba = new zzba();
        if (!TextUtils.isEmpty(this.zzqu)) {
            com_google_android_gms_internal_ads_zzba.zzcw = this.zzqu;
        }
        zzcz zzb = zzb(context, this.zzqt);
        if (zzb.isInitialized()) {
            zzdf zza;
            try {
                zza = zza(zzb, this.zzpy, this.zzqq);
                com_google_android_gms_internal_ads_zzba.zzdo = zza.zzsp;
                com_google_android_gms_internal_ads_zzba.zzdp = zza.zzsq;
                com_google_android_gms_internal_ads_zzba.zzdq = zza.zzsr;
                if (this.zzqp) {
                    com_google_android_gms_internal_ads_zzba.zzec = zza.zzfr;
                    com_google_android_gms_internal_ads_zzba.zzed = zza.zzfp;
                }
            } catch (zzcw e) {
            }
            zzbb com_google_android_gms_internal_ads_zzbb = new zzbb();
            if (this.zzqa > 0 && zzdg.zza(this.zzqq)) {
                com_google_android_gms_internal_ads_zzbb.zzfz = Long.valueOf(zzdg.zza(this.zzqh, this.zzqq));
                com_google_android_gms_internal_ads_zzbb.zzga = Long.valueOf(zzdg.zza((double) (this.zzqm - this.zzqk), this.zzqq));
                com_google_android_gms_internal_ads_zzbb.zzgb = Long.valueOf(zzdg.zza((double) (this.zzqn - this.zzql), this.zzqq));
                com_google_android_gms_internal_ads_zzbb.zzge = Long.valueOf(zzdg.zza((double) this.zzqk, this.zzqq));
                com_google_android_gms_internal_ads_zzbb.zzgf = Long.valueOf(zzdg.zza((double) this.zzql, this.zzqq));
                if (this.zzqp && this.zzpy != null) {
                    long zza2 = zzdg.zza((double) (((this.zzqk - this.zzqm) + this.zzpy.getRawX()) - this.zzpy.getX()), this.zzqq);
                    if (zza2 != 0) {
                        com_google_android_gms_internal_ads_zzbb.zzgc = Long.valueOf(zza2);
                    }
                    zza2 = zzdg.zza((double) (((this.zzql - this.zzqn) + this.zzpy.getRawY()) - this.zzpy.getY()), this.zzqq);
                    if (zza2 != 0) {
                        com_google_android_gms_internal_ads_zzbb.zzgd = Long.valueOf(zza2);
                    }
                }
            }
            try {
                zzdf zzb2 = zzb(this.zzpy);
                com_google_android_gms_internal_ads_zzbb.zzdo = zzb2.zzsp;
                com_google_android_gms_internal_ads_zzbb.zzdp = zzb2.zzsq;
                com_google_android_gms_internal_ads_zzbb.zzfu = zzb2.zzsr;
                if (this.zzqp) {
                    com_google_android_gms_internal_ads_zzbb.zzfp = zzb2.zzfp;
                    com_google_android_gms_internal_ads_zzbb.zzfr = zzb2.zzfr;
                    com_google_android_gms_internal_ads_zzbb.zzft = Integer.valueOf(zzb2.zzss.longValue() != 0 ? 1 : 0);
                    if (this.zzqb > 0) {
                        com_google_android_gms_internal_ads_zzbb.zzfq = zzdg.zza(this.zzqq) ? Long.valueOf(Math.round(((double) this.zzqg) / ((double) this.zzqb))) : null;
                        com_google_android_gms_internal_ads_zzbb.zzfs = Long.valueOf(Math.round(((double) this.zzqf) / ((double) this.zzqb)));
                    }
                    com_google_android_gms_internal_ads_zzbb.zzfw = zzb2.zzfw;
                    com_google_android_gms_internal_ads_zzbb.zzfv = zzb2.zzfv;
                    com_google_android_gms_internal_ads_zzbb.zzfx = Integer.valueOf(zzb2.zzsv.longValue() != 0 ? 1 : 0);
                }
            } catch (zzcw e2) {
            }
            if (this.zzqe > 0) {
                com_google_android_gms_internal_ads_zzbb.zzfy = Long.valueOf(this.zzqe);
            }
            com_google_android_gms_internal_ads_zzba.zzet = com_google_android_gms_internal_ads_zzbb;
            if (this.zzqa > 0) {
                com_google_android_gms_internal_ads_zzba.zzeh = Long.valueOf(this.zzqa);
            }
            if (this.zzqb > 0) {
                com_google_android_gms_internal_ads_zzba.zzeg = Long.valueOf(this.zzqb);
            }
            if (this.zzqc > 0) {
                com_google_android_gms_internal_ads_zzba.zzef = Long.valueOf(this.zzqc);
            }
            if (this.zzqd > 0) {
                com_google_android_gms_internal_ads_zzba.zzei = Long.valueOf(this.zzqd);
            }
            try {
                int size = this.zzpz.size() - 1;
                if (size > 0) {
                    com_google_android_gms_internal_ads_zzba.zzeu = new zzbb[size];
                    for (int i = 0; i < size; i++) {
                        zza = zza(zzps, (MotionEvent) this.zzpz.get(i), this.zzqq);
                        zzbb com_google_android_gms_internal_ads_zzbb2 = new zzbb();
                        com_google_android_gms_internal_ads_zzbb2.zzdo = zza.zzsp;
                        com_google_android_gms_internal_ads_zzbb2.zzdp = zza.zzsq;
                        com_google_android_gms_internal_ads_zzba.zzeu[i] = com_google_android_gms_internal_ads_zzbb2;
                    }
                }
            } catch (zzcw e3) {
                com_google_android_gms_internal_ads_zzba.zzeu = null;
            }
            ArrayList arrayList = new ArrayList();
            if (zzb.zzab() != null) {
                int zzx = zzb.zzx();
                arrayList.add(new zzdt(zzb, com_google_android_gms_internal_ads_zzba));
                arrayList.add(new zzdw(zzb, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", com_google_android_gms_internal_ads_zzba, zzx, 1));
                arrayList.add(new zzdr(zzb, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", com_google_android_gms_internal_ads_zzba, startTime, zzx, 25));
                arrayList.add(new zzdq(zzb, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", com_google_android_gms_internal_ads_zzba, zzx, 44));
                arrayList.add(new zzdv(zzb, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", com_google_android_gms_internal_ads_zzba, zzx, 12));
                arrayList.add(new zzdx(zzb, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", com_google_android_gms_internal_ads_zzba, zzx, 3));
                arrayList.add(new zzdu(zzb, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", com_google_android_gms_internal_ads_zzba, zzx, 22));
                arrayList.add(new zzdp(zzb, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", com_google_android_gms_internal_ads_zzba, zzx, 5));
                arrayList.add(new zzee(zzb, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", com_google_android_gms_internal_ads_zzba, zzx, 48));
                arrayList.add(new zzdm(zzb, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", com_google_android_gms_internal_ads_zzba, zzx, 49));
                arrayList.add(new zzeb(zzb, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", com_google_android_gms_internal_ads_zzba, zzx, 51));
                arrayList.add(new zzea(zzb, "nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", com_google_android_gms_internal_ads_zzba, zzx, 45, new Throwable().getStackTrace()));
                arrayList.add(new zzef(zzb, "j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", com_google_android_gms_internal_ads_zzba, zzx, 57, view));
                arrayList.add(new zzdz(zzb, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", com_google_android_gms_internal_ads_zzba, zzx, 61));
                if (((Boolean) zzkb.zzik().zzd(zznk.zzbar)).booleanValue()) {
                    arrayList.add(new zzdl(zzb, "wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", com_google_android_gms_internal_ads_zzba, zzx, 62, view, activity));
                }
                if (((Boolean) zzkb.zzik().zzd(zznk.zzbat)).booleanValue()) {
                    arrayList.add(new zzec(zzb, "aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", com_google_android_gms_internal_ads_zzba, zzx, 53, this.zzqx));
                }
            }
            Object obj = arrayList;
        } else {
            com_google_android_gms_internal_ads_zzba.zzdu = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            asList = Arrays.asList(new Callable[]{new zzdt(zzb, com_google_android_gms_internal_ads_zzba)});
        }
        zza(asList);
        return com_google_android_gms_internal_ads_zzba;
    }

    protected final zzba zza(Context context, zzax com_google_android_gms_internal_ads_zzax) {
        zzba com_google_android_gms_internal_ads_zzba = new zzba();
        if (!TextUtils.isEmpty(this.zzqu)) {
            com_google_android_gms_internal_ads_zzba.zzcw = this.zzqu;
        }
        zzcz zzb = zzb(context, this.zzqt);
        if (zzb.zzab() != null) {
            zza(zza(zzb, com_google_android_gms_internal_ads_zzba, null));
        }
        return com_google_android_gms_internal_ads_zzba;
    }

    protected List<Callable<Void>> zza(zzcz com_google_android_gms_internal_ads_zzcz, zzba com_google_android_gms_internal_ads_zzba, zzax com_google_android_gms_internal_ads_zzax) {
        int zzx = com_google_android_gms_internal_ads_zzcz.zzx();
        ArrayList arrayList = new ArrayList();
        if (com_google_android_gms_internal_ads_zzcz.isInitialized()) {
            ArrayList arrayList2 = arrayList;
            arrayList2.add(new zzdo(com_google_android_gms_internal_ads_zzcz, "LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", com_google_android_gms_internal_ads_zzba, zzx, 27, com_google_android_gms_internal_ads_zzax));
            arrayList2 = arrayList;
            arrayList2.add(new zzdr(com_google_android_gms_internal_ads_zzcz, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", com_google_android_gms_internal_ads_zzba, startTime, zzx, 25));
            arrayList2 = arrayList;
            arrayList2.add(new zzdw(com_google_android_gms_internal_ads_zzcz, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", com_google_android_gms_internal_ads_zzba, zzx, 1));
            arrayList2 = arrayList;
            arrayList2.add(new zzdy(com_google_android_gms_internal_ads_zzcz, "+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", com_google_android_gms_internal_ads_zzba, zzx, 31));
            arrayList2 = arrayList;
            arrayList2.add(new zzed(com_google_android_gms_internal_ads_zzcz, "R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", com_google_android_gms_internal_ads_zzba, zzx, 33));
            arrayList2 = arrayList;
            arrayList2.add(new zzdn(com_google_android_gms_internal_ads_zzcz, "1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", com_google_android_gms_internal_ads_zzba, zzx, 29));
            arrayList2 = arrayList;
            arrayList2.add(new zzdp(com_google_android_gms_internal_ads_zzcz, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", com_google_android_gms_internal_ads_zzba, zzx, 5));
            arrayList2 = arrayList;
            arrayList2.add(new zzdv(com_google_android_gms_internal_ads_zzcz, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", com_google_android_gms_internal_ads_zzba, zzx, 12));
            arrayList2 = arrayList;
            arrayList2.add(new zzdx(com_google_android_gms_internal_ads_zzcz, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", com_google_android_gms_internal_ads_zzba, zzx, 3));
            arrayList2 = arrayList;
            arrayList2.add(new zzdq(com_google_android_gms_internal_ads_zzcz, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", com_google_android_gms_internal_ads_zzba, zzx, 44));
            arrayList2 = arrayList;
            arrayList2.add(new zzdu(com_google_android_gms_internal_ads_zzcz, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", com_google_android_gms_internal_ads_zzba, zzx, 22));
            arrayList2 = arrayList;
            arrayList2.add(new zzee(com_google_android_gms_internal_ads_zzcz, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", com_google_android_gms_internal_ads_zzba, zzx, 48));
            arrayList2 = arrayList;
            arrayList2.add(new zzdm(com_google_android_gms_internal_ads_zzcz, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", com_google_android_gms_internal_ads_zzba, zzx, 49));
            arrayList2 = arrayList;
            arrayList2.add(new zzeb(com_google_android_gms_internal_ads_zzcz, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", com_google_android_gms_internal_ads_zzba, zzx, 51));
            arrayList2 = arrayList;
            arrayList2.add(new zzdz(com_google_android_gms_internal_ads_zzcz, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", com_google_android_gms_internal_ads_zzba, zzx, 61));
            return arrayList;
        }
        com_google_android_gms_internal_ads_zzba.zzdu = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
        return arrayList;
    }

    protected final zzdf zzb(MotionEvent motionEvent) throws zzcw {
        Throwable e;
        Method zza = zzps.zza("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=");
        if (zza == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new zzdf((String) zza.invoke(null, new Object[]{motionEvent, this.zzqq}));
        } catch (IllegalAccessException e2) {
            e = e2;
            throw new zzcw(e);
        } catch (InvocationTargetException e3) {
            e = e3;
            throw new zzcw(e);
        }
    }

    public final void zzb(View view) {
        if (!((Boolean) zzkb.zzik().zzd(zznk.zzbat)).booleanValue()) {
            return;
        }
        if (this.zzqx == null) {
            this.zzqx = new zzdi(zzps, view);
        } else {
            this.zzqx.zzd(view);
        }
    }
}
