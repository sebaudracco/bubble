...
public class GcmReceiver extends WakefulBroadcastReceiver {
    ...
    private final synchronized zzk zzd(Context context, String str) {
        zzk com_google_android_gms_iid_zzk;
        if ("com.google.android.c2dm.intent.RECEIVE".equals(str)) {
            if (zzs == null) {
                zzs = new zzk(context, str);
            }
            com_google_android_gms_iid_zzk = zzs;
        } else {
            if (zzr == null) {
                zzr = new zzk(context, str);
            }
            com_google_android_gms_iid_zzk = zzr;
        }
        return com_google_android_gms_iid_zzk;
    }
    ...
    private final void zzd(Context context, Intent intent) {
        ...
        if (resolveService == null || resolveService.serviceInfo == null) {
            ...
            if (!context.getPackageName().equals(serviceInfo.packageName) || serviceInfo.name == null) {
                ...
                Log.e("GcmReceiver", new StringBuilder((String.valueOf(str).length() + 94) + String.valueOf(str2).length()).append("Error resolving target intent service, skipping classname enforcement. Resolved service was: ").append(str).append(BridgeUtil.SPLIT_MARK).append(str2).toString());
            } else {
                String valueOf;
                str2 = serviceInfo.name;
                if (str2.startsWith(".")) {
