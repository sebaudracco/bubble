        ...
        for (Entry entry : map.entrySet()) {
            ...
            if (field2 != null) {
                try {
                    ...
                    zzane.zzdk(new StringBuilder(String.valueOf(str).length() + 49).append("Server option \"").append(str).append("\" could not be set: Illegal Access").toString());
                } catch (IllegalArgumentException e2) {
                    str = (String) entry.getKey();
                    zzane.zzdk(new StringBuilder(String.valueOf(str).length() + 43).append("Server option \"").append(str).append("\" could not be set: Bad Type").toString());
                }
            } else {
                String str2 = (String) entry.getKey();
                str = (String) entry.getValue();
                zzane.zzck(new StringBuilder((String.valueOf(str2).length() + 31) + String.valueOf(str).length()).append("Unexpected server option: ").append(str2).append(" = \"").append(str).append("\"").toString());
            }
        }
