...
public class FileLoadOperation {
    ...
    class C48551 implements Runnable {
        C48551() {
        }
    ...
    class C48562 implements Runnable {
        C48562() {
        }
    ...
    class C48584 implements RequestDelegate {
        C48584() {
        }
    ...
    public String getFileName() {
        if (this.location != null) {
            return this.location.volume_id + BridgeUtil.UNDERLINE_STR + this.location.local_id + "." + this.ext;
        }
    ...
    public boolean start() {
        ...
        if (this.webLocation != null) {
            ...
            if (this.datacenter_id == 0 || this.location.id == 0) {
                ...
                fileNameTemp = this.datacenter_id + BridgeUtil.UNDERLINE_STR + this.location.id + ".temp.enc";
                fileNameFinal = this.datacenter_id + BridgeUtil.UNDERLINE_STR + this.location.id + this.ext + ".enc";
                if (this.key != null) {
                    fileNameIv = this.datacenter_id + BridgeUtil.UNDERLINE_STR + this.location.id + ".iv.enc";
                }
            } else {
                fileNameTemp = this.datacenter_id + BridgeUtil.UNDERLINE_STR + this.location.id + ".temp";
                fileNameFinal = this.datacenter_id + BridgeUtil.UNDERLINE_STR + this.location.id + this.ext;
                if (this.key != null) {
                    fileNameIv = this.datacenter_id + BridgeUtil.UNDERLINE_STR + this.location.id + ".iv";
                }
            }
            ...
            fileNameTemp = this.location.volume_id + BridgeUtil.UNDERLINE_STR + this.location.local_id + ".temp.enc";
            fileNameFinal = this.location.volume_id + BridgeUtil.UNDERLINE_STR + this.location.local_id + "." + this.ext + ".enc";
            if (this.key != null) {
                fileNameIv = this.location.volume_id + BridgeUtil.UNDERLINE_STR + this.location.local_id + ".iv.enc";
            }
        } else {
            fileNameTemp = this.location.volume_id + BridgeUtil.UNDERLINE_STR + this.location.local_id + ".temp";
            fileNameFinal = this.location.volume_id + BridgeUtil.UNDERLINE_STR + this.location.local_id + "." + this.ext;
            if (this.key != null) {
                fileNameIv = this.location.volume_id + BridgeUtil.UNDERLINE_STR + this.location.local_id + ".iv";
            }
        }
        ...
        if (this.cacheFileFinal.exists()) {
            ...
            this.started = true;
            Utilities.stageQueue.postRunnable(new C48551());
        }
        return true;
    }

    public void cancel() {
        Utilities.stageQueue.postRunnable(new C48562());
    }
    ...
    private void requestFileOffsets(int offset) {
        if (!this.requestingCdnOffsets) {
            ...
            req.offset = offset;
            ConnectionsManager.getInstance().sendRequest(req, new C48584(), null, null, 0, this.datacenter_id, 1, true);
        }
    }
            ...
            try {
                ...
                while (a < this.delayedRequestInfos.size()) {
                    ...
                    if (this.downloadedBytes == delayedRequestInfo.offset) {
                        ...
                        } else if (delayedRequestInfo.response != null) {
                            delayedRequestInfo.response.disableFree = false;
                            delayedRequestInfo.response.freeResources();
                        } else if (delayedRequestInfo.responseWeb != null) {
                            delayedRequestInfo.responseWeb.disableFree = false;
        ...
        if ((this.totalBytesCount <= 0 || this.nextDownloadOffset < this.totalBytesCount) && this.requestInfos.size() + this.delayedRequestInfos.size() < this.currentMaxDownloadRequests) {
            ...
            while (a < count) {
                if (this.totalBytesCount <= 0 || this.nextDownloadOffset < this.totalBytesCount) {
                    ...
                    RequestDelegate c48616 = new RequestDelegate() {

                        class C48601 implements RequestDelegate {
                            C48601() {
                            }
                        ...
                        public void run(TLObject response, TLRPC$TL_error error) {
                            ...
                            if (error != null && (request instanceof TLRPC$TL_upload_getCdnFile) && error.text.equals("FILE_TOKEN_INVALID")) {
                                ...
                                if (res.encryption_iv == null || res.encryption_key == null || res.encryption_iv.length != 16 || res.encryption_key.length != 32) {
                                    ...
                                    error.code = HttpStatus.SC_BAD_REQUEST;
                                ...
                                ConnectionsManager.getInstance().sendRequest(req, new C48601(), null, null, 0, FileLoadOperation.this.datacenter_id, 1, true);
                            }
                        }
                    };
                    if (this.isCdn) {
                    ...
                    requestInfo.requestToken = instance.sendRequest(request, c48616, null, null, flags, i, connectionType, isLast);
                    this.requestsCount++;
                    a++;
                } else {
                    return;
