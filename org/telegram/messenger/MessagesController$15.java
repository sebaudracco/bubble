...
class MessagesController$15 implements RequestDelegate {
    ...
    class C50072 implements Runnable {
        C50072() {
        }
        ...
        if (error == null) {
            AndroidUtilities.runOnUIThread(new Runnable() {
                public void run() {
                    ...
                    if (userFull.bot_info instanceof TLRPC$TL_botInfo) {
                        NotificationCenter.getInstance().postNotificationName(NotificationCenter.botInfoDidLoaded, userFull.bot_info, Integer.valueOf(MessagesController$15.this.val$classGuid));
                    }
                    NotificationCenter.getInstance().postNotificationName(NotificationCenter.userInfoDidLoaded, Integer.valueOf(MessagesController$15.this.val$user.id), userFull);
                }
            });
        } else {
            AndroidUtilities.runOnUIThread(new C50072());
        }
    }
}
