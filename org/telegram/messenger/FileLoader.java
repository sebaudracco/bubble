    ...
    public void uploadFile(String location, boolean encrypted, boolean small, int estimatedSize, int type) {
        if (location != null) {
            ...
            this.fileLoaderQueue.postRunnable(new Runnable() {
                ...
                class C48661 implements FileUploadOperationDelegate {

                    class C48652 implements Runnable {
                        C48652() {
                        }
                    ...
                    public void didFailedUploadingFile(FileUploadOperation operation) {
                        FileLoader.this.fileLoaderQueue.postRunnable(new C48652());
                    }
                ...
                public void run() {
                    ...
                    operation.setDelegate(new C48661());
                    if (z2) {
                        if (FileLoader.this.currentUploadSmallOperationsCount < 1) {
                            FileLoader.this.currentUploadSmallOperationsCount = FileLoader.this.currentUploadSmallOperationsCount + 1;
                            operation.start();
    ...
    public static TLRPC$PhotoSize getClosestPhotoSizeWithSize(ArrayList<TLRPC$PhotoSize> sizes, int side, boolean byMinSide) {
        ...
        for (int a = 0; a < sizes.size(); a++) {
            ...
            if (obj != null) {
                int currentSide;
                if (byMinSide) {
                    currentSide = obj.f12636h >= obj.f12637w ? obj.f12637w : obj.f12636h;
                    ...
                    currentSide = obj.f12637w >= obj.f12636h ? obj.f12637w : obj.f12636h;
    ...
    public static String getAttachFileName(TLObject attach, String ext) {
        ...
        if (attach instanceof TLRPC$Document) {
            ...
            if (document.version == 0) {
                if (docExt.length() > 1) {
                    return document.dc_id + BridgeUtil.UNDERLINE_STR + document.id + docExt;
                }
                return document.dc_id + BridgeUtil.UNDERLINE_STR + document.id;
            } else if (docExt.length() > 1) {
                return document.dc_id + BridgeUtil.UNDERLINE_STR + document.id + BridgeUtil.UNDERLINE_STR + document.version + docExt;
            } else {
                return document.dc_id + BridgeUtil.UNDERLINE_STR + document.id + BridgeUtil.UNDERLINE_STR + document.version;
            }
        } else if (attach instanceof TLRPC$TL_webDocument) {
            TLRPC$TL_webDocument document2 = (TLRPC$TL_webDocument) attach;
            ...
            r5 = new StringBuilder().append(photo.location.volume_id).append(BridgeUtil.UNDERLINE_STR).append(photo.location.local_id).append(".");
            if (ext == null) {
                ext = "jpg";
            }
            ...
            r5 = new StringBuilder().append(location.volume_id).append(BridgeUtil.UNDERLINE_STR).append(location.local_id).append(".");
            if (ext == null) {
                ext = "jpg";
            }
            return r5.append(ext).toString();
