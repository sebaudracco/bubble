...
public class MessageObject {
        ...
        public void calculate() {
            ...
            if (count > 1) {
                ...
                while (a < count) {
                    ...
                    position.aspectRatio = photoSize == null ? 1.0f : ((float) photoSize.f12637w) / ((float) photoSize.f12636h);
    ...
    public MessageObject(TLRPC$Message message, AbstractMap<Integer, User> users, AbstractMap<Integer, TLRPC$Chat> chats, boolean generateLayout, long eid) {
        ...
        if (message instanceof TLRPC$TL_messageService) {
            if (message.action != null) {
                if (message.action instanceof TLRPC$TL_messageActionCustomAction) {
                    ...
                    this.messageText = LocaleController.formatString("NotificationContactJoined", R.string.NotificationContactJoined, new Object[]{UserObject.getUserName(fromUser), ApplicationLoader.getConfig().getAppName()});
            ...
            this.messageText = LocaleController.getAppNameString("UnsupportedMedia", R.string.UnsupportedMedia);
        ...
        if (generateLayout) {
            ...
            this.messageText = Emoji.replaceEmoji(this.messageText, paint.getFontMetricsInt(), AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP), false, emojiOnly);
    ...
    public MessageObject(TLRPC$TL_channelAdminLogEvent event, ArrayList<MessageObject> messageObjects, HashMap<String, ArrayList<MessageObject>> messagesByDays, TLRPC$Chat chat, int[] mid) {
        ...
        if (event.action instanceof TLRPC$TL_channelAdminLogEventActionChangeTitle) {
            ...
            if (event.action.new_photo instanceof TLRPC$TL_chatPhotoEmpty) {
                ...
                photoSize.type = RequestParams.f9036M;
                photoSize.h = 640;
                photoSize.w = 640;
                this.messageOwner.action.photo.sizes.add(photoSize);
                if (chat.megagroup) {
            ...
            if (TextUtils.isEmpty(newLink)) {
                ...
                message.message = "https://" + MessagesController.getInstance().linkPrefix + BridgeUtil.SPLIT_MARK + newLink;
            }
            ...
            if (TextUtils.isEmpty(((TLRPC$TL_channelAdminLogEventActionChangeUsername) event.action).prev_value)) {
                ...
                message.media.webpage.description = "https://" + MessagesController.getInstance().linkPrefix + BridgeUtil.SPLIT_MARK + ((TLRPC$TL_channelAdminLogEventActionChangeUsername) event.action).prev_value;
            }
    ...
    public void applyNewText() {
        if (!TextUtils.isEmpty(this.messageOwner.message)) {
            ...
            this.messageText = Emoji.replaceEmoji(this.messageText, paint.getFontMetricsInt(), AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP), false);
            generateLayout(fromUser);
        }
    }
    ...
    public void generatePinMessageText(User fromUser, TLRPC$Chat chat) {
        ...
        if (this.replyMessageObject == null || (this.replyMessageObject.messageOwner instanceof TLRPC$TL_messageEmpty) || (this.replyMessageObject.messageOwner.action instanceof TLRPC$TL_messageActionHistoryClear)) {
            ...
            this.messageText = replaceWithLink(string, str, fromUser);
            this.messageText = Emoji.replaceEmoji(this.messageText, Theme.chat_msgTextPaint.getFontMetricsInt(), AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP), false);
            ...
            mess = Emoji.replaceEmoji(mess, Theme.chat_msgTextPaint.getFontMetricsInt(), AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP), false);
            ...
            for (int a = 0; a < this.messageOwner.reply_markup.rows.size(); a++) {
                ...
                for (int b = 0; b < size; b++) {
                    ...
                    if (!(button instanceof TLRPC$TL_keyboardButtonBuy) || (this.messageOwner.media.flags & 4) == 0) {
                        text = Emoji.replaceEmoji(button.text, Theme.chat_msgBotButtonPaint.getFontMetricsInt(), AndroidUtilities.dp(CtaButton.TEXT_SIZE_SP), false);
                    } else {
        ...
        if (!(document == null || document.mime_type == null || !document.mime_type.equals("video/mp4"))) {
            ...
            for (int a = 0; a < document.attributes.size(); a++) {
                ...
                if (attribute instanceof TLRPC$TL_documentAttributeVideo) {
                    width = attribute.f12630w;
                    height = attribute.f12630w;
            ...
            if (this.messageOwner.media instanceof TLRPC$TL_messageMediaPhoto) {
                ...
                if (!(this.messageOwner.media.document.thumb instanceof TLRPC$TL_photoSizeEmpty)) {
                    if (!update) {
                        ...
                        photoObject.f12637w = this.messageOwner.media.document.thumb.f12637w;
                        photoObject.f12636h = this.messageOwner.media.document.thumb.f12636h;
                    }
                }
            } else if (this.messageOwner.media instanceof TLRPC$TL_messageMediaGame) {
    ...
    public CharSequence replaceWithLink(CharSequence source, String param, TLObject object) {
        ...
        if (object instanceof User) {
            ...
            name = "";
            id = SchemaSymbols.ATTVAL_FALSE_0;
        }
    ...
    public void generateLinkDescription() {
        if (this.linkDescription == null) {
            ...
            if (this.linkDescription != null) {
                ...
                this.linkDescription = Emoji.replaceEmoji(this.linkDescription, Theme.chat_msgTextPaint.getFontMetricsInt(), AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP), false);
            }
        }
    }
    ...
    public void generateCaption() {
        if (this.caption == null && !isRoundVideo() && this.messageOwner.media != null && !TextUtils.isEmpty(this.messageOwner.media.caption)) {
            this.caption = Emoji.replaceEmoji(this.messageOwner.media.caption, Theme.chat_msgTextPaint.getFontMetricsInt(), AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP), false);
    ...
    public void generateLayout(User fromUser) {
        if (this.type == 0 && this.messageOwner.to_id != null && !TextUtils.isEmpty(this.messageText)) {
            ...
            if ((fromUser != null && fromUser.bot) || ((isMegagroup() || !(this.messageOwner.fwd_from == null || this.messageOwner.fwd_from.channel_id == 0)) && !isOut())) {
                maxWidth -= AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP);
            }
            if (this.messageOwner.media instanceof TLRPC$TL_messageMediaGame) {
            ...
            if (hasUrls) {
                try {
                    if (VERSION.SDK_INT >= 24) {
                        ...
                        blocksCount = (int) Math.ceil((double) (((float) linesCount) / Settings.LOCATION_REQUEST_SMALLEST_DISPLACEMENT));
                        linesOffset = 0;
                        prevOffset = 0.0f;
    ...
    public String getSecretTimeString() {
        ...
        return (secondsLeft / 60) + RequestParams.f9036M;
    }
    ...
    public int getApproximateHeight() {
        if (this.type == 0) {
            ...
            if (this.type == 13) {
                ...
                while (it.hasNext()) {
                    ...
                    if (attribute instanceof TLRPC$TL_documentAttributeImageSize) {
                        photoWidth = attribute.f12630w;
                        photoHeight = attribute.f12629h;
                        break;
                    }
                }
                if (photoWidth == 0) {
            ...
            if (currentPhotoObject != null) {
                int h = (int) (((float) currentPhotoObject.f12636h) / (((float) currentPhotoObject.f12637w) / ((float) photoWidth)));
                if (h == 0) {
