...
public class ImageLoader {
    ...
    class C48972 implements FileLoaderDelegate {
        C48972() {
        }
        ...
        public void fileDidUploaded(String location, TLRPC$InputFile inputFile, TLRPC$InputEncryptedFile inputEncryptedFile, byte[] key, byte[] iv, long totalFileSize) {
            ...
            Utilities.stageQueue.postRunnable(new Runnable() {
                ...
                class C48901 implements Runnable {
                    C48901() {
                    }

                    public void run() {
                        NotificationCenter.getInstance().postNotificationName(NotificationCenter.FileDidUpload, str, tLRPC$InputFile, tLRPC$InputEncryptedFile, bArr, bArr2, Long.valueOf(j));
                    }
                }

                public void run() {
                    AndroidUtilities.runOnUIThread(new C48901());
        ...
        public void fileDidFailedUpload(final String location, final boolean isEncrypted) {
            Utilities.stageQueue.postRunnable(new Runnable() {
                ...
                class C48921 implements Runnable {
                    C48921() {
                    }
                ...
                public void run() {
                    AndroidUtilities.runOnUIThread(new C48921());
    ...
    class C48993 extends BroadcastReceiver {

        class C48981 implements Runnable {
            C48981() {
            }
    ...
    class C49014 implements Runnable {
        C49014() {
        }
        ...
        private void reportProgress(final float progress) {
            ...
            if (progress == 1.0f || this.lastProgressTime == 0 || this.lastProgressTime < currentTime - 500) {
                ...
                Utilities.stageQueue.postRunnable(new Runnable() {
                    ...
                    class C49101 implements Runnable {
                        C49101() {
                        }
                    ...
                    public void run() {
                        ...
                        AndroidUtilities.runOnUIThread(new C49101());
                    }
                });
            }
        }
            ...
            try {
                ...
                if (httpConnection instanceof HttpURLConnection) {
                    ...
                    if (status == 302 || status == 301 || status == 303) {
                        String newUrl = httpURLConnection.getHeaderField(HttpHeaders.LOCATION);
                        String cookies = httpURLConnection.getHeaderField(SM.SET_COOKIE);
                        httpConnection = new URL(newUrl).openConnection();
                        httpConnection.setRequestProperty(SM.COOKIE, cookies);
                        httpConnection.addRequestProperty("User-Agent", "Mozilla/5.0 (iPhone; CPU iPhone OS 10_0 like Mac OS X) AppleWebKit/602.1.38 (KHTML, like Gecko) Version/10.0 Mobile/14A5297c Safari/602.1");
                    }
                }
    ...
    private class HttpImageTask extends AsyncTask<Void, Void, Boolean> {
        ...
        class C49163 implements Runnable {
            C49163() {
            }
        ...
        class C49174 implements Runnable {
            C49174() {
            }
        ...
        class C49195 implements Runnable {

            class C49181 implements Runnable {
                C49181() {
                }
            ...
            public void run() {
                ...
                AndroidUtilities.runOnUIThread(new C49181());
            }
        }
            ...
            if (progress == 1.0f || this.lastProgressTime == 0 || this.lastProgressTime < currentTime - 500) {
                ...
                Utilities.stageQueue.postRunnable(new Runnable() {
                    ...
                    public void run() {
                        ...
                        AndroidUtilities.runOnUIThread(new C49121());
                    }
                });
            }
        }
            ...
            if (!isCancelled()) {
                try {
                    if (this.httpConnection != null && (this.httpConnection instanceof HttpURLConnection)) {
                        ...
                        if (!(code == 200 || code == HttpStatus.SC_ACCEPTED || code == HttpStatus.SC_NOT_MODIFIED)) {
                            this.canRetry = false;
                        }
                    }
                } catch (Exception e2) {
        ...
        protected void onPostExecute(final Boolean result) {
            ...
            Utilities.stageQueue.postRunnable(new Runnable() {
                ...
                class C49141 implements Runnable {
                    C49141() {
                    }
                ...
                public void run() {
                    ImageLoader.this.fileProgresses.remove(HttpImageTask.this.cacheImage.url);
                    AndroidUtilities.runOnUIThread(new C49141());
                }
            });
            ImageLoader.this.imageLoadQueue.postRunnable(new C49163());
        }

        protected void onCancelled() {
            ImageLoader.this.imageLoadQueue.postRunnable(new C49174());
            Utilities.stageQueue.postRunnable(new C49195());
        }
    }
    ...
    private class ThumbGenerateTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                final String key = this.thumbLocation.volume_id + BridgeUtil.UNDERLINE_STR + this.thumbLocation.local_id;
    ...
    public ImageLoader() {
        ...
        this.memCache = new LruCache((Math.min(15, ((ActivityManager) ApplicationLoader.applicationContext.getSystemService(C1404b.aw)).getMemoryClass() / 7) * 1024) * 1024) {
        ...
        FileLoader.getInstance().setDelegate(new C48972());
        BroadcastReceiver receiver = new C48993();
    ...
    public void checkMediaPaths() {
        this.cacheOutQueue.postRunnable(new C49014());
    }
    ...
    public BitmapDrawable getImageFromMemory(TLObject fileLocation, String httpUrl, String filter) {
        ...
        if (httpUrl != null) {
            ...
            key = location.volume_id + BridgeUtil.UNDERLINE_STR + location.local_id;
        } else if (fileLocation instanceof TLRPC$Document) {
            TLRPC$Document location2 = (TLRPC$Document) fileLocation;
            if (location2.version == 0) {
                key = location2.dc_id + BridgeUtil.UNDERLINE_STR + location2.id;
            } else {
                key = location2.dc_id + BridgeUtil.UNDERLINE_STR + location2.id + BridgeUtil.UNDERLINE_STR + location2.version;
            }
    ...
    public void loadImageForImageReceiver(ImageReceiver imageReceiver) {
        if (imageReceiver != null) {
                ...
                if (imageLocation instanceof TLRPC$FileLocation) {
                    ...
                    if (document2.id != 0 && document2.dc_id != 0) {
                        if (document2.version == 0) {
                            key = document2.dc_id + BridgeUtil.UNDERLINE_STR + document2.id;
                        } else {
                            key = document2.dc_id + BridgeUtil.UNDERLINE_STR + document2.id + BridgeUtil.UNDERLINE_STR + document2.version;
                        }
                        ...
                        if (docExt != null) {
                            ...
                            if (idx != -1) {
                                ...
                                if (docExt.length() <= 1) {
                                    if (document2.mime_type == null && document2.mime_type.equals("video/mp4")) {
                                        docExt = ".mp4";
                                    } else {
                                        docExt = "";
                                    }
            ...
            if (thumbLocation != null) {
                thumbKey = thumbLocation.volume_id + BridgeUtil.UNDERLINE_STR + thumbLocation.local_id;
    ...
    private static TLRPC$PhotoSize scaleAndSaveImageInternal(Bitmap bitmap, int w, int h, float photoW, float photoH, float scaleFactor, int quality, boolean cache, boolean scaleAnyway) throws Exception {
        ...
        size.f12637w = scaledBitmap.getWidth();
        size.f12636h = scaledBitmap.getHeight();
        if (size.f12637w <= 100 && size.f12636h <= 100) {
            size.type = "s";
        } else if (size.f12637w <= 320 && size.f12636h <= 320) {
            size.type = RequestParams.f9036M;
        } else if (size.f12637w <= 800 && size.f12636h <= 800) {
            size.type = "x";
        } else if (size.f12637w > 1280 || size.f12636h > 1280) {
            size.type = "w";
        } else {
            size.type = "y";
        }
        FileOutputStream stream = new FileOutputStream(new File(FileLoader.getInstance().getDirectory(4), location.volume_id + BridgeUtil.UNDERLINE_STR + location.local_id + ".jpg"));
    ...
    public static void saveMessageThumbs(TLRPC$Message message) {
        ...
        if (photoSize != null && photoSize.bytes != null && photoSize.bytes.length != 0) {
            ...
            newPhotoSize.w = photoSize.f12637w;
            newPhotoSize.h = photoSize.f12636h;
