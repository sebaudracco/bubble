...
public class MusicPlayerService extends Service implements NotificationCenterDelegate {
    ...
    public void onCreate() {
        ...
        if (VERSION.SDK_INT >= 21) {
            ...
            this.mediaSession.setCallback(new C50951());
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (messageObject == null) {
            AndroidUtilities.runOnUIThread(new C50962());
            return 1;
        }
    ...
    private void createNotification(MessageObject messageObject) {
        ...
        if (VERSION.SDK_INT >= 21) {
            ...
            PendingIntent pendingPrev = PendingIntent.getBroadcast(getApplicationContext(), 0, new Intent(NOTIFY_PREVIOUS).setComponent(new ComponentName(this, MusicPlayerReceiver.class)), ErrorDialogData.BINDER_CRASH);
            PendingIntent pendingStop = PendingIntent.getService(getApplicationContext(), 0, new Intent(this, getClass()).setAction(getPackageName() + ".STOP_PLAYER"), ErrorDialogData.BINDER_CRASH);
            PendingIntent pendingPlaypause = PendingIntent.getBroadcast(getApplicationContext(), 0, new Intent(isPlaying ? NOTIFY_PAUSE : NOTIFY_PLAY).setComponent(new ComponentName(this, MusicPlayerReceiver.class)), ErrorDialogData.BINDER_CRASH);
            PendingIntent pendingNext = PendingIntent.getBroadcast(getApplicationContext(), 0, new Intent(NOTIFY_NEXT).setComponent(new ComponentName(this, MusicPlayerReceiver.class)), ErrorDialogData.BINDER_CRASH);
