...
class ContactsController$9 implements Runnable {
    ...
    class C48401 implements Runnable {
        C48401() {
        }
    ...
    public void run() {
        ...
        if (!this.val$contactHashMap.isEmpty()) {
            ...
            if (!this.val$first && this.val$contactHashMap.isEmpty() && toImport.isEmpty() && oldCount == contactsMap.size()) {
                ...
                if (!(true || this.val$contactHashMap.isEmpty())) {
                    AndroidUtilities.runOnUIThread(new C48401());
                }
            }
        } else if (this.val$request) {
        ...
        if (!this.val$request) {
            ...
            Utilities.stageQueue.postRunnable(new Runnable() {
                ...
                class C48481 implements Runnable {
                    C48481() {
                    }

                    public void run() {
                        ContactsController.access$1200(ContactsController$9.this.this$0, ContactsController$9.this.this$0.contacts);
                ...
                public void run() {
                    ...
                    AndroidUtilities.runOnUIThread(new C48481());
                }
            });
        } else {
            int checkType;
            ...
            if (checkType != 0) {
                AndroidUtilities.runOnUIThread(new Runnable() {
                    public void run() {
                        NotificationCenter.getInstance().postNotificationName(NotificationCenter.hasNewContactsToImport, Integer.valueOf(checkType), ContactsController$9.this.val$contactHashMap, Boolean.valueOf(ContactsController$9.this.val$first), Boolean.valueOf(ContactsController$9.this.val$schedule));
                    }
                });
            } else if (this.val$canceled) {
                Utilities.stageQueue.postRunnable(new Runnable() {

                    class C48421 implements Runnable {
                        C48421() {
                        }
                    ...
                    public void run() {
                        ...
                        MessagesStorage.getInstance().putCachedPhoneBook(contactsMap, false);
                        AndroidUtilities.runOnUIThread(new C48421());
                    }
                });
            } else {
                ...
                for (a = 0; a < count; a++) {
                    ...
                    int start = a * HttpStatus.SC_INTERNAL_SERVER_ERROR;
                    req.contacts = new ArrayList(toImport.subList(start, Math.min(start + HttpStatus.SC_INTERNAL_SERVER_ERROR, toImport.size())));
                    ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate() {
                        ...
                        class C48461 implements Runnable {
                            ...
                            class C48452 implements Runnable {
                                C48452() {
                                }
                            ...
                            public void run() {
                                ...
                                AndroidUtilities.runOnUIThread(new C48441());
                                if (hasErrors[0]) {
                                    Utilities.globalQueue.postRunnable(new C48452(), 5000);
                                }
                            }
                        }
                        ...
                        public void run(TLObject response, TLRPC$TL_error error) {
                            ...
                            if (ContactsController.access$100(ContactsController$9.this.this$0) == count) {
                                ...
                                Utilities.stageQueue.postRunnable(new C48461());
                            }
                        }
                    }, 6);
                }
