...
class MessagesController$67 implements Runnable {
    ...
    public void run() {
        ...
        if (this.val$loadType == 1 && this.val$dialogsRes.dialogs.size() == 0) {
            AndroidUtilities.runOnUIThread(new C50321());
            return;
        }
        int a;
        TLRPC$Chat chat;
        ...
        AndroidUtilities.runOnUIThread(new Runnable() {
            public void run() {
                ...
                if (!(MessagesController$67.this.val$fromCache || MessagesController$67.this.val$migrate || UserConfig.totalDialogsLoadCount >= HttpStatus.SC_BAD_REQUEST || UserConfig.dialogsLoadOffsetId == -1 || UserConfig.dialogsLoadOffsetId == Integer.MAX_VALUE)) {
