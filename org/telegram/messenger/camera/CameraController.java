...
public class CameraController implements OnInfoListener {
    ...
    class C52181 implements Runnable {

        class C52131 implements Comparator<Size> {
            C52131() {
            }
        ...
        class C52142 implements Runnable {
            C52142() {
            }
        ...
        class C52153 implements Runnable {
            C52153() {
            }
            ...
            try {
                if (CameraController.this.cameraInfos == null) {
                    ...
                    for (int cameraId = 0; cameraId < count; cameraId++) {
                        ...
                        Comparator<Size> comparator = new C52131();
                ...
                AndroidUtilities.runOnUIThread(new C52142());
            } catch (Exception e) {
                AndroidUtilities.runOnUIThread(new C52153());
                FileLog.e(e);
            }
        }
    }

    class C52192 implements Runnable {
        C52192() {
        }
    ...
    public void initCamera() {
        if (!this.loadingCameras && !this.cameraInitied) {
            ...
            this.threadPool.execute(new C52181());
        }
    }
    ...
    public void cleanup() {
        this.threadPool.execute(new C52192());
    }
                ...
                public void run() {
                    ...
                    if (rawFlashModes != null) {
                        for (int a = 0; a < rawFlashModes.size(); a++) {
                            ...
                            if (rawFlashMode.equals(C1042c.jG) || rawFlashMode.equals(C1042c.jF) || rawFlashMode.equals("auto")) {
                ...
                public void run() {
                    try {
                        if (camera != null) {
                            try {
                                Parameters params = camera.getParameters();
                                params.setFlashMode(cameraSession.getCurrentFlashMode().equals(C1042c.jF) ? "torch" : C1042c.jG);
                            ...
                            try {
                                ...
                                if (CameraController.this.recordingSmallVideo) {
                                    pictureSize = CameraController.chooseOptimalSize(info.getPictureSizes(), 640, SizeUtils.DEFAULT_INTERSTITIAL_HEIGHT, new Size(4, 3));
                                    ...
                                    pictureSize = CameraController.chooseOptimalSize(info.getPictureSizes(), 720, SizeUtils.DEFAULT_INTERSTITIAL_HEIGHT, new Size(16, 9));
                                    CameraController.this.recorder.setVideoEncodingBitRate(1800000);
                                }
