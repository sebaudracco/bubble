...
public class SearchQuery {
    ...
    static class C53442 implements Runnable {
        C53442() {
        }
    ...
    static class C53483 implements RequestDelegate {
        C53483() {
        }
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    public void run() {
                        ...
                        MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable() {
                            ...
                            class C53451 implements Runnable {
                                C53451() {
                                }
                            ...
                            public void run() {
                                try {
                                    ...
                                    AndroidUtilities.runOnUIThread(new C53451());
                                } catch (Exception e) {
                                    FileLog.e(e);
                                }
                            }
    ...
    static class C53494 implements Comparator<TLRPC$TL_topPeer> {
        C53494() {
        }
    ...
    static class C53505 implements RequestDelegate {
        C53505() {
        }
    ...
    static class C53516 implements RequestDelegate {
        C53516() {
        }
    ...
    public static void loadHints(boolean cache) {
        if (!loading) {
            if (!cache) {
                ...
                req.offset = 0;
                req.limit = 20;
                ConnectionsManager.getInstance().sendRequest(req, new C53483());
            } else if (!loaded) {
                loading = true;
                MessagesStorage.getInstance().getStorageQueue().postRunnable(new C53442());
                loaded = true;
            }
        }
    }
    ...
    public static void increaseInlineRaiting(int uid) {
        ...
        Collections.sort(inlineBots, new C53494());
    ...
    public static void removeInline(int uid) {
        for (int a = 0; a < inlineBots.size(); a++) {
            if (((TLRPC$TL_topPeer) inlineBots.get(a)).peer.user_id == uid) {
                ...
                ConnectionsManager.getInstance().sendRequest(req, new C53505());
                deletePeer(uid, 1);
                NotificationCenter.getInstance().postNotificationName(NotificationCenter.reloadInlineHints, new Object[0]);
                return;
            }
    ...
    public static void removePeer(int uid) {
        for (int a = 0; a < hints.size(); a++) {
            if (((TLRPC$TL_topPeer) hints.get(a)).peer.user_id == uid) {
                ...
                req.peer = MessagesController.getInputPeer(uid);
                deletePeer(uid, 0);
                ConnectionsManager.getInstance().sendRequest(req, new C53516());
                return;
            }
        }
    }
                ...
                MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable() {
                    public void run() {
                        ...
                        AndroidUtilities.runOnUIThread(new Runnable() {
                            ...
                            class C53521 implements Comparator<TLRPC$TL_topPeer> {
                                C53521() {
                                }
                            ...
                            public void run() {
                                ...
                                Collections.sort(SearchQuery.hints, new C53521());
