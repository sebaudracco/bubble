    ...
    private static void processLoadedMedia(TLRPC$messages_Messages res, long uid, int count, int max_id, int type, boolean fromCache, int classGuid, boolean isChannel, boolean topReached) {
        ...
        AndroidUtilities.runOnUIThread(new Runnable() {
            public void run() {
                ...
                NotificationCenter.getInstance().postNotificationName(NotificationCenter.mediaDidLoaded, Long.valueOf(j), Integer.valueOf(totalCount), objects, Integer.valueOf(i), Integer.valueOf(i2), Boolean.valueOf(z2));
            }
        });
    }
    ...
    public static void loadMusic(final long uid, final long max_id) {
        MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable() {
            public void run() {
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    public void run() {
                        NotificationCenter.getInstance().postNotificationName(NotificationCenter.musicDidLoaded, Long.valueOf(uid), arrayList);
                    }
                });
            }
        });
