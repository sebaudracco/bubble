...
public class MessagesQuery {
    private static Comparator<TLRPC$MessageEntity> entityComparator = new C53261();

    static class C53261 implements Comparator<TLRPC$MessageEntity> {
        C53261() {
        }
    ...
    private static MessageObject broadcastPinnedMessage(TLRPC$Message result, ArrayList<User> users, ArrayList<TLRPC$Chat> chats, boolean isCache, boolean returnValue) {
        ...
        AndroidUtilities.runOnUIThread(new Runnable() {
            public void run() {
                ...
                NotificationCenter.getInstance().postNotificationName(NotificationCenter.didLoadedPinnedMessage, new MessageObject(tLRPC$Message, usersDict, chatsDict, false));
            }
        });
        return null;
    }
    ...
    public static void loadReplyMessagesForMessages(ArrayList<MessageObject> messages, long dialogId) {
                ...
                MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            AndroidUtilities.runOnUIThread(new C53311());
                        } catch (Exception e) {
                            FileLog.e(e);
                        }
                    }
        ...
        if (!replyMessages2.isEmpty()) {
            ...
            MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable() {
                ...
                class C53331 implements RequestDelegate {
                    C53331() {
                    }
                ...
                class C53342 implements RequestDelegate {
                    C53342() {
                    }
                    ...
                    try {
                        ...
                        if (!replyMessages2.isEmpty()) {
                            if (channelIdFinal != 0) {
                                ...
                                ConnectionsManager.getInstance().sendRequest(req, new C53331());
                                return;
                            }
                            TLRPC$TL_messages_getMessages req2 = new TLRPC$TL_messages_getMessages();
                            req2.id = replyMessages2;
                            ConnectionsManager.getInstance().sendRequest(req2, new C53342());
                        }
                    } catch (Exception e) {
                        FileLog.e(e);
                    }
