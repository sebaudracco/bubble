...
class SendMessagesHelper$20 implements Runnable {
    ...
    SendMessagesHelper$20(long j, VideoEditedInfo videoEditedInfo, String str, long j2, int i, int i2, int i3, long j3, CharSequence charSequence, MessageObject messageObject) {
    ...
    public void run() {
        ...
        if (this.val$videoEditedInfo != null || this.val$videoPath.endsWith("mp4") || isRound) {
            ...
            long startTime = 0;
            originalPath = originalPath + file.length() + BridgeUtil.UNDERLINE_STR + file.lastModified();
            if (this.val$videoEditedInfo != null) {
                if (!isRound) {
                    originalPath = originalPath + this.val$duration + BridgeUtil.UNDERLINE_STR + this.val$videoEditedInfo.startTime + BridgeUtil.UNDERLINE_STR + this.val$videoEditedInfo.endTime + (this.val$videoEditedInfo.muted ? "_m" : "");
                    if (this.val$videoEditedInfo.resultWidth == this.val$videoEditedInfo.originalWidth) {
                        originalPath = originalPath + BridgeUtil.UNDERLINE_STR + this.val$videoEditedInfo.resultWidth;
                    }
                }
            ...
            if (tLRPC$TL_document == null) {
                ...
                if (!(thumb == null || size == null)) {
                    if (!isRound) {
                        ...
                        thumbKey = String.format(size.location.volume_id + BridgeUtil.UNDERLINE_STR + size.location.local_id + "@%d_%d_b2", new Object[]{Integer.valueOf((int) (((float) AndroidUtilities.roundMessageSize) / AndroidUtilities.density)), Integer.valueOf((int) (((float) AndroidUtilities.roundMessageSize) / AndroidUtilities.density))});
                    } else {
                        Utilities.blurBitmap(thumb, 3, VERSION.SDK_INT < 21 ? 0 : 1, thumb.getWidth(), thumb.getHeight(), thumb.getRowBytes());
                        thumbKey = String.format(size.location.volume_id + BridgeUtil.UNDERLINE_STR + size.location.local_id + "@%d_%d_b", new Object[]{Integer.valueOf((int) (((float) AndroidUtilities.roundMessageSize) / AndroidUtilities.density)), Integer.valueOf((int) (((float) AndroidUtilities.roundMessageSize) / AndroidUtilities.density))});
                    }
                }
            ...
            AndroidUtilities.runOnUIThread(new Runnable() {
                public void run() {
                    ...
                    SendMessagesHelper.getInstance().sendMessage(videoFinal, SendMessagesHelper$20.this.val$videoEditedInfo, finalPath, SendMessagesHelper$20.this.val$dialog_id, SendMessagesHelper$20.this.val$reply_to_msg, null, params, SendMessagesHelper$20.this.val$ttl);
                }
            });
            return;
        }
