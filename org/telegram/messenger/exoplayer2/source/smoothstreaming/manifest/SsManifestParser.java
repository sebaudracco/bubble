...
public class SsManifestParser implements Parser<SsManifest> {
    ...
    private static class QualityLevelParser extends ElementParser {
        ...
        public void parseStartTag(XmlPullParser parser) throws ParserException {
            ...
            if (type == 2) {
                this.format = Format.createVideoContainerFormat(id, "video/mp4", sampleMimeType, null, bitrate, parseRequiredInt(parser, KEY_MAX_WIDTH), parseRequiredInt(parser, KEY_MAX_HEIGHT), -1.0f, buildCodecSpecificData(parser.getAttributeValue(null, KEY_CODEC_PRIVATE_DATA)), 0);
            } else if (type == 1) {
                ...
                String str = id;
                this.format = Format.createAudioContainerFormat(str, "audio/mp4", sampleMimeType, null, bitrate, channels, samplingRate, codecSpecificData, 0, (String) getNormalizedAttribute(KEY_LANGUAGE));
            } else if (type == 3) {
                this.format = Format.createTextContainerFormat(id, "application/mp4", sampleMimeType, null, bitrate, 0, (String) getNormalizedAttribute(KEY_LANGUAGE));
            } else {
                this.format = Format.createContainerFormat(id, "application/mp4", sampleMimeType, null, bitrate, 0, null);
            }
        }
    ...
    private static class SmoothStreamingMediaParser extends ElementParser {
        ...
        public Object build() {
            ...
            if (this.protectionElement != null) {
                DrmInitData drmInitData = new DrmInitData(new SchemeData[]{new SchemeData(this.protectionElement.uuid, null, "video/mp4", this.protectionElement.data)});
    ...
    private static class StreamIndexParser extends ElementParser {
        ...
        private void parseStreamFragmentStartTag(XmlPullParser parser) throws ParserException {
            ...
            long startTime = parseLong(parser, KEY_FRAGMENT_START_TIME, -9223372036854775807L);
            ...
            this.lastChunkDuration = parseLong(parser, KEY_FRAGMENT_DURATION, -9223372036854775807L);
            long repeatCount = parseLong(parser, KEY_FRAGMENT_REPEAT_COUNT, 1);
            if (repeatCount <= 1 || this.lastChunkDuration != -9223372036854775807L) {
