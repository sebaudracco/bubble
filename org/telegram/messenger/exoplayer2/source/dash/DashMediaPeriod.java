    ...
    private static Pair<TrackGroupArray, TrackGroupInfo[]> buildTrackGroups(List<AdaptationSet> adaptationSets) {
        ...
        while (i < primaryGroupCount) {
            ...
            if (hasEventMessageTrack) {
                Format format = Format.createSampleFormat(firstAdaptationSet.id + ":emsg", "application/x-emsg", null, -1, null);
            ...
            if (hasCea608Track) {
                format = Format.createTextSampleFormat(firstAdaptationSet.id + ":cea608", "application/cea-608", 0, null);
