...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new C54081();

    static class C54081 implements Comparator<Snake> {
        C54081() {
        }

        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f12622x - o2.f12622x;
            return cmpX == 0 ? o1.f12623y - o2.f12623y : cmpX;
        }
    }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f12622x != 0 || firstSnake.f12623y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f12622x + snake.size;
                int endY = snake.f12623y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f12622x + j;
                    int newItemPos = snake.f12623y + j;
                ...
                posOld = snake.f12622x;
                posNew = snake.f12623y;
            }
        }
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f12622x + snakeSize;
                int endY = snake.f12623y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f12622x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f12622x + i, 1, this.mCallback.getChangePayload(snake.f12622x + i, snake.f12623y + i));
                    }
                }
                posOld = snake.f12622x;
                posNew = snake.f12623y;
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f12622x += range.oldListStart;
                snake.f12623y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f12622x;
                    left.newListEnd = snake.f12623y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f12622x - 1;
                    left.newListEnd = snake.f12623y;
                } else {
                    left.oldListEnd = snake.f12622x;
                    left.newListEnd = snake.f12623y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f12622x + snake.size;
                    right.newListStart = snake.f12623y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = (snake.f12622x + snake.size) + 1;
                    right.newListStart = snake.f12623y + snake.size;
                } else {
                    right.oldListStart = snake.f12622x + snake.size;
                    right.newListStart = (snake.f12623y + snake.size) + 1;
                }
                stack.add(right);
            } else {
                rangePool.add(range);
        ...
        while (d <= dLimit) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f12622x = backward[kOffset + k];
                    outSnake.f12623y = outSnake.f12622x - k;
            ...
            while (k <= d) {
                ...
                if (checkInFwd || k + delta < (-d) || k + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake = new Snake();
                    outSnake.f12622x = backward[kOffset + backwardK];
                    outSnake.f12623y = outSnake.f12622x - backwardK;
