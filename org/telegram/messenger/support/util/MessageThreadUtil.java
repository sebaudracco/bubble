...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C54101();
            final MessageQueue mQueue = new MessageQueue();

            class C54101 implements Runnable {
                C54101() {
                }

                public void run() {
                    SyncQueueItem msg = C54111.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C54121();
            AtomicBoolean mBackgroundRunning = new AtomicBoolean(false);
            private final Executor mExecutor = ParallelExecutorCompat.getParallelExecutor();
            final MessageQueue mQueue = new MessageQueue();

            class C54121 implements Runnable {
                C54121() {
                }

                public void run() {
                    while (true) {
                        SyncQueueItem msg = C54132.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C54132.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C54132.this.mQueue.removeMessages(2);
                                    C54132.this.mQueue.removeMessages(3);
                        ...
                        C54132.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
