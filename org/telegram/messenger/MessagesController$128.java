...
class MessagesController$128 implements Runnable {
    ...
    class C49993 implements RequestDelegate {
        C49993() {
        }

        public void run(TLObject response, TLRPC$TL_error error) {
            if (response != null) {
                MessagesController$128.this.this$0.processUpdates((TLRPC$Updates) response, false);
            }
        }
    }

    MessagesController$128(MessagesController this$0, int i, ArrayList arrayList, HashMap hashMap, HashMap hashMap2, HashMap hashMap3, boolean z, ArrayList arrayList2, ArrayList arrayList3, SparseArray sparseArray) {
        this.this$0 = this$0;
    ...
    public void run() {
                    ...
                    if (chat != null) {
                        if (dialog == null && (chat instanceof TLRPC$TL_channel) && !chat.left) {
                            ...
                            Utilities.stageQueue.postRunnable(new Runnable() {
                                public void run() {
                                    MessagesController$128.this.this$0.getChannelDifference(tLRPC$Update.channel_id, 1, 0, null);
                                }
                            });
                    ...
                    if (call instanceof TLRPC$TL_phoneCallRequested) {
                        if (call.date + (this.this$0.callRingTimeout / 1000) >= ConnectionsManager.getInstance().getCurrentTime()) {
                            ...
                            if (svc == null && VoIPService.callIShouldHavePutIntoIntent == null && tm.getCallState() == 0) {
                                ...
                                ConnectionsManager.getInstance().sendRequest(req, new C49993());
                            }
        ...
        if (!this.val$webPages.isEmpty()) {
            NotificationCenter.getInstance().postNotificationName(NotificationCenter.didReceivedWebpagesInUpdates, this.val$webPages);
        ...
        if (this.val$channelViews.size() != 0) {
            NotificationCenter.getInstance().postNotificationName(NotificationCenter.didUpdatedMessagesViews, this.val$channelViews);
        }
        if (updateMask != 0) {
