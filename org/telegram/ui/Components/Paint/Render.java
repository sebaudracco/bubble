...
public class Render {
    ...
    private static void PaintSegment(Point lastPoint, Point point, RenderState state) {
        ...
        Point unitVector = new Point(MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE, MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE, 0.0d);
        if (Math.abs(state.angle) > 0.0f) {
            vectorAngle = state.angle;
        } else {
            vectorAngle = (float) Math.atan2(vector.f12661y, vector.f12660x);
        }
        float brushWeight = state.baseWeight * state.scale;
        double step = (double) Math.max(1.0f, state.spacing * brushWeight);
        if (distance > 0.0d) {
            unitVector = vector.multiplyByScalar(MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE / distance);
        }
