...
public class TextPaintView extends EntityView {
    ...
    class C61661 implements TextWatcher {
    ...
    public class TextViewSelectionView extends SelectionView {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            float inset = (radius + thickness) + ((float) AndroidUtilities.dp(CtaButton.TEXT_SIZE_SP));
    ...
    public TextPaintView(Context context, Point position, int fontSize, String text, Swatch swatch, boolean stroke) {
        ...
        setStroke(stroke);
        updatePosition();
        this.editText.addTextChangedListener(new C61661());
    }
    ...
    protected Rect getSelectionBounds() {
        ...
        float height = (((float) getHeight()) * getScale()) + (((float) AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP)) / scale);
        return new Rect((this.position.f12664x - (width / 2.0f)) * scale, (this.position.f12665y - (height / 2.0f)) * scale, width * scale, height * scale);
    }
