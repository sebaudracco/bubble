...
public class EntityView extends FrameLayout {
    ...
    class C61651 extends SimpleOnGestureListener {
        C61651() {
        }
    ...
    public class SelectionView extends FrameLayout {
        ...
        protected void updatePosition() {
            ...
            layoutParams.leftMargin = ((int) bounds.f12666x) + EntityView.this.offsetX;
            layoutParams.topMargin = ((int) bounds.f12667y) + EntityView.this.offsetY;
                ...
                case 2:
                    if (this.currentHandle != 3) {
                        if (this.currentHandle != 0) {
                            ...
                            float delta = (float) ((((double) translation.f12664x) * Math.cos((double) radAngle)) + (((double) translation.f12665y) * Math.sin((double) radAngle)));
    ...
    public EntityView(Context context, Point pos) {
        super(context);
        this.position = pos;
        this.gestureDetector = new GestureDetector(context, new C61651());
    }
    ...
    private boolean onTouchMove(float x, float y) {
        ...
        if (((float) Math.hypot((double) translation.f12664x, (double) translation.f12665y)) <= (this.hasPanned ? 6.0f : 16.0f)) {
            return false;
        }
        pan(translation);
    ...
    public void pan(Point translation) {
        ...
        point.f12664x += translation.f12664x;
        point = this.position;
        point.f12665y += translation.f12665y;
        updatePosition();
    }

    protected void updatePosition() {
        float halfHeight = ((float) getHeight()) / 2.0f;
        setX(this.position.f12664x - (((float) getWidth()) / 2.0f));
        setY(this.position.f12665y - halfHeight);
        updateSelectionView();
    }
