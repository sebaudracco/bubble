...
public class TypingDotsDrawable extends StatusDrawable {
    ...
    class C63131 implements Runnable {
        C63131() {
        }
    ...
    public void draw(Canvas canvas) {
        ...
        canvas.drawCircle((float) AndroidUtilities.dp(CtaButton.TEXT_SIZE_SP), (float) y, this.scales[2] * AndroidUtilities.density, Theme.chat_statusPaint);
        checkUpdate();
    }
    ...
    private void checkUpdate() {
        ...
        if (NotificationCenter.getInstance().isAnimationInProgress()) {
            AndroidUtilities.runOnUIThread(new C63131(), 100);
        } else {
            update();
        }
    }
    ...
    public int getIntrinsicWidth() {
        return AndroidUtilities.dp(RadialCountdown.TEXT_SIZE_SP);
    }
