...
public class PlayingGameDrawable extends StatusDrawable {
    ...
    class C62351 implements Runnable {
        C62351() {
        }
    ...
    public void draw(Canvas canvas) {
        int rad;
        int size = AndroidUtilities.dp(Settings.LOCATION_REQUEST_SMALLEST_DISPLACEMENT);
    ...
    private void checkUpdate() {
        ...
        if (NotificationCenter.getInstance().isAnimationInProgress()) {
            AndroidUtilities.runOnUIThread(new C62351(), 100);
        } else {
            update();
        }
    }
    ...
    public int getIntrinsicWidth() {
        return AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP);
    }
