...
public class BotKeyboardView extends LinearLayout {
    ...
    class C60131 implements OnClickListener {
        C60131() {
        }
    ...
    public void setPanelHeight(int height) {
        ...
        if (this.isFullSize && this.botButtons != null && this.botButtons.rows.size() != 0) {
            int max;
            if (this.isFullSize) {
                max = (int) Math.max(42.0f, ((float) (((this.panelHeight - AndroidUtilities.dp(BitmapDescriptorFactory.HUE_ORANGE)) - ((this.botButtons.rows.size() - 1) * AndroidUtilities.dp(Settings.LOCATION_REQUEST_SMALLEST_DISPLACEMENT))) / this.botButtons.rows.size())) / AndroidUtilities.density);
            } else {
                max = 42;
            }
    ...
    public void setButtons(TLRPC$TL_replyKeyboardMarkup buttons) {
        ...
        if (buttons != null && this.botButtons.rows.size() != 0) {
            ...
            this.buttonHeight = !this.isFullSize ? 42 : (int) Math.max(42.0f, ((float) (((this.panelHeight - AndroidUtilities.dp(BitmapDescriptorFactory.HUE_ORANGE)) - ((this.botButtons.rows.size() - 1) * AndroidUtilities.dp(Settings.LOCATION_REQUEST_SMALLEST_DISPLACEMENT))) / this.botButtons.rows.size())) / AndroidUtilities.density);
            int a = 0;
            while (a < buttons.rows.size()) {
                ...
                float f2 = a == 0 ? CtaButton.TEXT_SIZE_SP : Settings.LOCATION_REQUEST_SMALLEST_DISPLACEMENT;
                if (a == buttons.rows.size() - 1) {
                    f = CtaButton.TEXT_SIZE_SP;
                } else {
                    f = 0.0f;
                }
                linearLayout.addView(layout, LayoutHelper.createLinear(-1, i, CtaButton.TEXT_SIZE_SP, f2, CtaButton.TEXT_SIZE_SP, f));
                ...
                while (b < row.buttons.size()) {
                    ...
                    textView.setOnClickListener(new C60131());
    ...
    public int getKeyboardHeight() {
        return this.isFullSize ? this.panelHeight : ((this.botButtons.rows.size() * AndroidUtilities.dp((float) this.buttonHeight)) + AndroidUtilities.dp(BitmapDescriptorFactory.HUE_ORANGE)) + ((this.botButtons.rows.size() - 1) * AndroidUtilities.dp(Settings.LOCATION_REQUEST_SMALLEST_DISPLACEMENT));
    }
}
