...
public class VoIPHelper {
            ...
            if (VoIPService.getSharedInstance() != null) {
                if (VoIPService.getSharedInstance().getUser().id != user.id) {
                    new Builder(activity).setTitle(LocaleController.getString("VoipOngoingAlertTitle", R.string.VoipOngoingAlertTitle)).setMessage(AndroidUtilities.replaceTags(LocaleController.formatString("VoipOngoingAlert", R.string.VoipOngoingAlert, new Object[]{ContactsController.formatName(callUser.first_name, callUser.last_name), ContactsController.formatName(user.first_name, user.last_name)}))).setPositiveButton(LocaleController.getString("OK", R.string.OK), new OnClickListener() {
                        ...
                        class C63371 implements Runnable {
                            C63371() {
                            }
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            if (VoIPService.getSharedInstance() != null) {
                                VoIPService.getSharedInstance().hangUp(new C63371());
                            } else {
                                VoIPHelper.doInitiateCall(user, activity);
                            }
                        }
    ...
    private static void doInitiateCall(User user, Activity activity) {
        if (activity != null && user != null && System.currentTimeMillis() - lastCallTime >= Reveal.CHECK_DELAY) {
    ...
    public static void permissionDenied(final Activity activity, final Runnable onFinish) {
        if (!activity.shouldShowRequestPermissionRationale("android.permission.RECORD_AUDIO")) {
            new Builder(activity).setTitle(ApplicationLoader.getConfig().getAppName()).setMessage(LocaleController.getAppNameString("VoipNeedMicPermission", R.string.VoipNeedMicPermission)).setPositiveButton(LocaleController.getString("OK", R.string.OK), null).setNegativeButton(LocaleController.getString("Settings", R.string.Settings), new OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
    ...
    public static void showRateAlert(Context context, Runnable onDismiss, long callID, long accessHash) {
        ...
        linearLayout.setText(LocaleController.getAppNameString("VoipRateCallAlert", R.string.VoipRateCallAlert));
        ...
        linearLayout.setTextSize(RadialCountdown.TEXT_SIZE_SP);
        ...
        View.OnClickListener c63415 = new View.OnClickListener() {
        ...
        linearLayout.setOnClickListener(c63415);
                    ...
                    public void run(TLObject response, TLRPC$TL_error error) {
                        if (response instanceof TLRPC$TL_updates) {
                            ...
                            if (includeLogs[0] && log.exists() && req.rating < 4) {
                                SendMessagesHelper.prepareSendingDocument(log.getAbsolutePath(), log.getAbsolutePath(), null, HTTP.PLAIN_TEXT_TYPE, 4244000, null, null);
                                Toast.makeText(context2, LocaleController.formatString("CallReportSent", R.string.CallReportSent, new Object[]{ApplicationLoader.getConfig().getAppName()}), 1).show();
                            }
                        }
                    }
                });
