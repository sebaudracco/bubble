...
public class CropView extends FrameLayout implements AreaViewListener, CropGestureListener {
    ...
    class C60581 implements OnPreDrawListener {
        C60581() {
        }
    ...
    class C60647 implements OnCancelListener {
        C60647() {
        }
    ...
    private class CropState {
        ...
        private boolean hasChanges() {
            return Math.abs(this.f12655x) > CropView.EPSILON || Math.abs(this.f12656y) > CropView.EPSILON || Math.abs(this.scale - this.minimumScale) > CropView.EPSILON || Math.abs(this.rotation) > CropView.EPSILON || Math.abs(this.orientation) > CropView.EPSILON;
        }
    ...
    public void setBitmap(Bitmap b, int rotation, boolean fform) {
        ...
        this.imageView.getViewTreeObserver().addOnPreDrawListener(new C60581());
    ...
    public void showAspectRatioDialog() {
        if (this.areaView.getLockAspectRatio() > 0.0f) {
            ...
            dialog.setOnCancelListener(new C60647());
            dialog.show();
        }
    }
