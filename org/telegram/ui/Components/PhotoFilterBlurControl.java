...
public class PhotoFilterBlurControl extends FrameLayout {
    private static final float BlurInsetProximity = ((float) AndroidUtilities.dp(CloseButton.TEXT_SIZE_SP));
            ...
            case 5:
                if (event.getPointerCount() == 1) {
                    if (this.checkForMoving && !this.isMoving) {
                        ...
                        Point delta = new Point(locationX - centerPoint.f12664x, locationY - centerPoint.f12665y);
                        float radialDistance = (float) Math.sqrt((double) ((delta.f12664x * delta.f12664x) + (delta.f12665y * delta.f12665y)));
                        ...
                        if (this.type == 0) {
                            float distance = (float) Math.abs((((double) delta.f12664x) * Math.cos(((double) degreesToRadians(this.angle)) + 1.5707963267948966d)) + (((double) delta.f12665y) * Math.sin(((double) degreesToRadians(this.angle)) + 1.5707963267948966d)));
    ...
    private void handlePan(int state, MotionEvent event) {
        ...
        Point delta = new Point(locationX - actualCenterPoint.f12664x, locationY - actualCenterPoint.f12665y);
        float radialDistance = (float) Math.sqrt((double) ((delta.f12664x * delta.f12664x) + (delta.f12665y * delta.f12665y)));
            ...
            case 2:
                    ...
                    if (this.type == 1) {
                        switch (this.activeControl) {
                            case BlurViewActiveControlCenter:
                                ...
                                point = new Point(Math.max(actualArea.f12666x, Math.min(actualArea.f12666x + actualArea.width, this.startCenterPoint.f12664x + translationX)), Math.max(actualArea.f12667y, Math.min(actualArea.f12667y + actualArea.height, this.startCenterPoint.f12665y + translationY)));
                                this.centerPoint = new Point((point.f12664x - actualArea.f12666x) / this.actualAreaSize.width, ((point.f12665y - actualArea.f12667y) + ((this.actualAreaSize.width - this.actualAreaSize.height) / 2.0f)) / this.actualAreaSize.width);
                                break;
                ...
                switch (this.activeControl) {
                    ...
                    case BlurViewActiveControlRotation:
                        ...
                        boolean right = locationX > actualCenterPoint.f12664x;
                        boolean bottom = locationY > actualCenterPoint.f12665y;
                        if (right || bottom) {
                            if (!right || bottom) {
                                if (right && bottom) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float innerRadius = getActualInnerRadius();
        float outerRadius = getActualOuterRadius();
        canvas.translate(centerPoint.f12664x, centerPoint.f12665y);
        int i;
        Canvas canvas2;
        if (this.type == 0) {
            float f;
    ...
    private Point getActualCenterPoint() {
        return new Point((this.centerPoint.f12664x * this.actualAreaSize.width) + ((((float) getWidth()) - this.actualAreaSize.width) / 2.0f), ((((float) (VERSION.SDK_INT >= 21 ? AndroidUtilities.statusBarHeight : 0)) + ((((float) getHeight()) - this.actualAreaSize.height) / 2.0f)) - ((this.actualAreaSize.width - this.actualAreaSize.height) / 2.0f)) + (this.centerPoint.f12665y * this.actualAreaSize.width));
    }
