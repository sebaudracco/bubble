...
public class Scroller {
    ...
    static {
        ...
        for (int i = 0; i <= 100; i++) {
            ...
            while (true) {
                ...
                if (((double) Math.abs(tx - t)) < PathInterpolatorCompat.EPSILON) {
                    break;
                } else if (tx > t) {
                    x_max = x;
                } else {
    ...
    public void fling(int startX, int startY, int velocityX, int velocityY, int minX, int maxX, int minY, int maxY) {
        ...
        this.mDuration = (int) (1000.0d * Math.exp(l / (((double) DECELERATION_RATE) - MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE)));
        this.mStartTime = AnimationUtils.currentAnimationTimeMillis();
        this.mStartX = startX;
        this.mStartY = startY;
        float coeffX = velocity == 0.0f ? 1.0f : ((float) velocityX) / velocity;
        float coeffY = velocity == 0.0f ? 1.0f : ((float) velocityY) / velocity;
        int totalDistance = (int) (((double) 1145569280) * Math.exp((((double) DECELERATION_RATE) / (((double) DECELERATION_RATE) - MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE)) * l));
        this.mMinX = minX;
        this.mMaxX = maxX;
        this.mMinY = minY;
        this.mMaxY = maxY;
