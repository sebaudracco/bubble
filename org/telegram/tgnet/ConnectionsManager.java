...
public class ConnectionsManager {
    ...
    public int sendRequest(TLObject object, RequestDelegate onComplete, QuickAckDelegate onQuickAck, WriteToSocketDelegate onWriteToSocket, int flags, int datacenterId, int connetionType, boolean immediate) {
        ...
        Utilities.stageQueue.postRunnable(new 1(this, object, requestToken, onComplete, onQuickAck, onWriteToSocket, flags, datacenterId, connetionType, immediate));
        return requestToken;
    }
    ...
    public void init(int version, int layer, int apiId, String deviceModel, String systemVersion, String appVersion, String langCode, String systemLangCode, String configPath, String logPath, int userId, boolean enablePushConnection) {
        ...
        checkConnection();
        ApplicationLoader.applicationContext.registerReceiver(new 2(this), new IntentFilter("android.net.conn.CONNECTIVITY_CHANGE"));
    }
    ...
    public void setAppPaused(boolean value, boolean byScreenState) {
        if (!byScreenState) {
            ...
            FileLog.m4941d("app paused = " + value);
            if (value) {
                this.appResumeCount--;
            } else {
                this.appResumeCount++;
            }
            FileLog.m4941d("app resume count " + this.appResumeCount);
        ...
        if (this.appResumeCount == 0) {
            ...
            FileLog.m4942e("reset app pause time");
    ...
    public static void onUnparsedMessageReceived(int address) {
        try {
            ...
            if (message instanceof Updates) {
                FileLog.m4941d("java received " + message);
                AndroidUtilities.runOnUIThread(new 3());
                Utilities.stageQueue.postRunnable(new 4(message));
            }
        } catch (Throwable e) {
            FileLog.m4944e(e);
        }
    ...
    public static void onUpdate() {
        Utilities.stageQueue.postRunnable(new 5());
    }

    public static void onSessionCreated() {
        Utilities.stageQueue.postRunnable(new 6());
    }

    public static void onConnectionStateChanged(int state) {
        if (Instance != null) {
            if (Instance.timer != null) {
                Log.w("proxy", "timer cancel");
                Instance.timer.cancel();
                Instance.timer = null;
            }
            Log.w("proxy", "state = " + state);
            if (state == 1) {
                Instance.timer = new Timer();
                Log.w("proxy", "timer start");
                Instance.timer.schedule(new 7(), 5000);
            } else if (state == 4 && isNetworkOnline()) {
                Log.w("proxy", "online");
                native_setNetworkAvailable(false, getCurrentNetworkType());
                native_setNetworkAvailable(true, getCurrentNetworkType());
            }
        }
        AndroidUtilities.runOnUIThread(new 8(state));
    }

    public static void onLogout() {
        AndroidUtilities.runOnUIThread(new 9());
    }
    ...
    public static void onRequestNewServerIpAndPort(int second) {
        ...
        if ((second == 1 || Math.abs(lastDnsRequestTime - System.currentTimeMillis()) >= 10000) && isNetworkOnline()) {
            ...
            if (second == 1) {
                DnsTxtLoadTask task = new DnsTxtLoadTask(null);
                task.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[]{null, null, null});
                currentTask = task;
                return;
            }
            DnsLoadTask task2 = new DnsLoadTask(null);
    ...
    public static void onUpdateConfig(int address) {
        try {
            ...
            if (message != null) {
                Utilities.stageQueue.postRunnable(new 10(message));
            }
        } catch (Throwable e) {
            FileLog.m4944e(e);
        }
    ...
    public static void onInternalPushReceived() {
        AndroidUtilities.runOnUIThread(new 11());
    }
    ...
    public void setIsUpdating(boolean value) {
        AndroidUtilities.runOnUIThread(new 12(this, value));
    }
            ...
            try {
                ...
                while (networkInterfaces.hasMoreElements()) {
                    ...
                    if (!(!networkInterface.isUp() || networkInterface.isLoopback() || networkInterface.getInterfaceAddresses().isEmpty())) {
                        FileLog.m4942e("valid interface: " + networkInterface);
