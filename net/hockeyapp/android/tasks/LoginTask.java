...
public class LoginTask extends ConnectionTask<Void, Void, Boolean> {
    ...
    private HttpURLConnection makeRequest(int mode, Map<String, String> params) throws IOException {
        if (mode == 1) {
            return new HttpURLConnectionBuilder(this.mUrlString).setRequestMethod(HttpPost.METHOD_NAME).writeFormFields(params).build();
        }
        if (mode == 2) {
            return new HttpURLConnectionBuilder(this.mUrlString).setRequestMethod(HttpPost.METHOD_NAME).setBasicAuthorization((String) params.get("email"), (String) params.get("password")).build();
        }
        if (mode == 3) {
            return new HttpURLConnectionBuilder(this.mUrlString + "?" + ((String) params.get("type")) + "=" + ((String) params.get("id"))).build();
        }
