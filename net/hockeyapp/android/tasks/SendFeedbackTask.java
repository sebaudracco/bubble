...
public class SendFeedbackTask extends ConnectionTask<Void, Void, HashMap<String, String>> {
    ...
    protected void onPreExecute() {
        String loadingMessage = this.mContext.getString(C4702R.string.hockeyapp_feedback_sending_feedback_text);
        if (this.mIsFetchMessages) {
            loadingMessage = this.mContext.getString(C4702R.string.hockeyapp_feedback_fetching_feedback_text);
        }
    ...
    private HashMap<String, String> doPostPut() {
        ...
        try {
            ...
            if (this.mToken != null) {
                this.mUrlString += this.mToken + BridgeUtil.SPLIT_MARK;
            }
            urlConnection = new HttpURLConnectionBuilder(this.mUrlString).setRequestMethod(this.mToken != null ? "PUT" : HttpPost.METHOD_NAME).writeFormFields(parameters).build();
    ...
    private HashMap<String, String> doPostPutWithAttachments() {
        ...
        try {
            ...
            urlConnection = new HttpURLConnectionBuilder(this.mUrlString).setRequestMethod(this.mToken != null ? "PUT" : HttpPost.METHOD_NAME).writeMultipartData(parameters, this.mContext, this.mAttachmentUris).build();
            urlConnection.connect();
            result.put("status", String.valueOf(urlConnection.getResponseCode()));
            result.put("response", ConnectionTask.getStringFromConnection(urlConnection));
            if (urlConnection != null) {
