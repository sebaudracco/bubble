...
public class Sender {
    ...
    class C47131 extends AsyncTask<Void, Void, Void> {
        C47131() {
        }
    ...
    protected void triggerSending() {
        if (requestCount() < 10) {
            try {
                AsyncTaskUtils.execute(new C47131());
                return;
            } catch (Throwable e) {
    ...
    protected boolean isRecoverableError(int responseCode) {
        return Arrays.asList(new Integer[]{Integer.valueOf(HttpStatus.SC_REQUEST_TIMEOUT), Integer.valueOf(429), Integer.valueOf(HttpStatus.SC_INTERNAL_SERVER_ERROR), Integer.valueOf(HttpStatus.SC_SERVICE_UNAVAILABLE), Integer.valueOf(FrameMetricsAggregator.EVERY_DURATION)}).contains(Integer.valueOf(responseCode));
    }

    protected boolean isExpected(int responseCode) {
        return 200 <= responseCode && responseCode <= HttpStatus.SC_NON_AUTHORITATIVE_INFORMATION;
    }
    ...
    protected Writer getWriter(HttpURLConnection connection) throws IOException {
        ...
        connection.addRequestProperty("Content-Encoding", AsyncHttpClient.ENCODING_GZIP);
        connection.setRequestProperty("Content-Type", "application/x-json-stream");
