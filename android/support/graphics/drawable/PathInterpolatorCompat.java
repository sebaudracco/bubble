...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (((double) Math.abs(this.mX[0])) > EPSILON || ((double) Math.abs(this.mY[0])) > EPSILON || ((double) Math.abs(this.mX[numPoints - 1] - 1.0f)) > EPSILON || ((double) Math.abs(this.mY[numPoints - 1] - 1.0f)) > EPSILON) {
