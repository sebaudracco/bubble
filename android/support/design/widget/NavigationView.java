...
public class NavigationView extends ScrimInsetsFrameLayout {
    ...
    public static class SavedState extends AbsSavedState {
        ...
        static class C00661 implements ClassLoaderCreator<SavedState> {
            C00661() {
            }
    ...
    public NavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0026R.styleable.NavigationView, defStyleAttr, C0026R.style.Widget_Design_NavigationView);
        ViewCompat.setBackground(this, a.getDrawable(C0026R.styleable.NavigationView_android_background));
        if (a.hasValue(C0026R.styleable.NavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a.getDimensionPixelSize(C0026R.styleable.NavigationView_elevation, 0));
        }
        ViewCompat.setFitsSystemWindows(this, a.getBoolean(C0026R.styleable.NavigationView_android_fitsSystemWindows, false));
        this.mMaxWidth = a.getDimensionPixelSize(C0026R.styleable.NavigationView_android_maxWidth, 0);
        if (a.hasValue(C0026R.styleable.NavigationView_itemIconTint)) {
            itemIconTint = a.getColorStateList(C0026R.styleable.NavigationView_itemIconTint);
        } else {
            itemIconTint = createDefaultColorStateList(16842808);
        }
        boolean textAppearanceSet = false;
        int textAppearance = 0;
        if (a.hasValue(C0026R.styleable.NavigationView_itemTextAppearance)) {
            textAppearance = a.getResourceId(C0026R.styleable.NavigationView_itemTextAppearance, 0);
            textAppearanceSet = true;
        }
        ColorStateList itemTextColor = null;
        if (a.hasValue(C0026R.styleable.NavigationView_itemTextColor)) {
            itemTextColor = a.getColorStateList(C0026R.styleable.NavigationView_itemTextColor);
        }
        if (!textAppearanceSet && itemTextColor == null) {
            itemTextColor = createDefaultColorStateList(16842806);
        }
        Drawable itemBackground = a.getDrawable(C0026R.styleable.NavigationView_itemBackground);
        this.mMenu.setCallback(new C00651());
        ...
        if (a.hasValue(C0026R.styleable.NavigationView_menu)) {
            inflateMenu(a.getResourceId(C0026R.styleable.NavigationView_menu, 0));
        }
        if (a.hasValue(C0026R.styleable.NavigationView_headerLayout)) {
            inflateHeaderView(a.getResourceId(C0026R.styleable.NavigationView_headerLayout, 0));
        }
        a.recycle();
    }
