...
class RoundRectDrawableWithShadow extends Drawable {
    ...
    RoundRectDrawableWithShadow(Resources resources, ColorStateList backgroundColor, float radius, float shadowSize, float maxShadowSize) {
        this.mShadowStartColor = resources.getColor(C0335R.color.cardview_shadow_start_color);
        this.mShadowEndColor = resources.getColor(C0335R.color.cardview_shadow_end_color);
        this.mInsetShadow = resources.getDimensionPixelSize(C0335R.dimen.cardview_compat_inset_shadow);
    ...
    static float calculateVerticalPadding(float maxShadowSize, float cornerRadius, boolean addPaddingForCorners) {
        if (addPaddingForCorners) {
            return (float) (((double) (SHADOW_MULTIPLIER * maxShadowSize)) + ((MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE - COS_45) * ((double) cornerRadius)));
        }
    ...
    static float calculateHorizontalPadding(float maxShadowSize, float cornerRadius, boolean addPaddingForCorners) {
        if (addPaddingForCorners) {
            return (float) (((double) maxShadowSize) + ((MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE - COS_45) * ((double) cornerRadius)));
        }
    ...
    private void buildShadowCorners() {
        ...
        this.mCornerShadowPath.arcTo(innerBounds, BitmapDescriptorFactory.HUE_VIOLET, RadialCountdown.START_ANGLE, false);
