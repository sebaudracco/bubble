    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                if (rootPath.endsWith(BridgeUtil.SPLIT_MARK)) {
                    path = path.substring(rootPath.length());
                } else {
                    path = path.substring(rootPath.length() + 1);
                }
                return new Builder().scheme(FirebaseAnalytics$Param.CONTENT).authority(this.mAuthority).encodedPath(Uri.encode((String) mostSpecific.getKey()) + '/' + Uri.encode(path, BridgeUtil.SPLIT_MARK)).build();
            } catch (IOException e) {
