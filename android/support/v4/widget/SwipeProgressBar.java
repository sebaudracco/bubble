...
final class SwipeProgressBar {
    ...
    void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            long iterations = (now - this.mStartTime) / Reveal.CHECK_DELAY;
            float rawProgress = ((float) ((now - this.mStartTime) % Reveal.CHECK_DELAY)) / CloseButton.TEXT_SIZE_SP;
            if (!this.mRunning) {
                if (now - this.mFinishTime >= 1000) {
                    this.mFinishTime = 0;
                    return;
                }
                float clearRadius = ((float) (width / 2)) * INTERPOLATOR.getInterpolation((((float) ((now - this.mFinishTime) % 1000)) / Settings.LOCATION_REQUEST_SMALLEST_DISPLACEMENT) / 100.0f);
            ...
            ViewCompat.postInvalidateOnAnimation(this.mParent, this.mBounds.left, this.mBounds.top, this.mBounds.right, this.mBounds.bottom);
        } else if (this.mTriggerPercentage > 0.0f && ((double) this.mTriggerPercentage) <= MoPubAdapter.DEFAULT_MOPUB_IMAGE_SCALE) {
            drawTrigger(canvas, cx, cy);
        }
        canvas.restoreToCount(restoreCount);
    }
