...
public final class Cookie {
    ...
    private static boolean pathMatch(HttpUrl url, String path) {
        ...
        if (urlPath.startsWith(path) && (path.endsWith(BridgeUtil.SPLIT_MARK) || urlPath.charAt(path.length()) == '/')) {
            return true;
        }
        return false;
    }
    ...
    static Cookie parse(long currentTimeMillis, HttpUrl url, String setCookie) {
        ...
        while (pos < limit) {
            ...
            if (attributeName.equalsIgnoreCase(ClientCookie.EXPIRES_ATTR)) {
                try {
                ...
                if (attributeName.equalsIgnoreCase("max-age")) {
                    ...
                    if (attributeName.equalsIgnoreCase(ClientCookie.DOMAIN_ATTR)) {
                        try {
                        ...
                        if (attributeName.equalsIgnoreCase(ClientCookie.PATH_ATTR)) {
                            path = attributeValue;
                        } else {
                            if (attributeName.equalsIgnoreCase(ClientCookie.SECURE_ATTR)) {
                                secureOnly = true;
                            } else {
        ...
        if (path == null || !path.startsWith(BridgeUtil.SPLIT_MARK)) {
            ...
            path = lastSlash != 0 ? encodedPath.substring(0, lastSlash) : BridgeUtil.SPLIT_MARK;
        }
    ...
    public static List<Cookie> parseAll(HttpUrl url, Headers headers) {
        List<String> cookieStrings = headers.values(SM.SET_COOKIE);
