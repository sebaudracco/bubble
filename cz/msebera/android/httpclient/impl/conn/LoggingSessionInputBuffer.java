...
public class LoggingSessionInputBuffer implements SessionInputBuffer, EofSensor {
    ...
    public String readLine() throws IOException {
        ...
        if (this.wire.enabled() && s != null) {
            this.wire.input((s + LineSeparator.Windows).getBytes(this.charset));
        }
        return s;
    }
    ...
    public int readLine(CharArrayBuffer buffer) throws IOException {
        ...
        if (this.wire.enabled() && l >= 0) {
            this.wire.input((new String(buffer.buffer(), buffer.length() - l, l) + LineSeparator.Windows).getBytes(this.charset));
        }
        return l;
    }
